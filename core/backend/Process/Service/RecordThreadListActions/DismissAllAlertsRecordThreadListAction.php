...
class DismissAllAlertsRecordThreadListAction implements ProcessHandlerInterface
{
    protected const MSG_OPTIONS_NOT_FOUND = 'Process options are not defined';

    protected const PROCESS_TYPE = 'record-thread-list-dismiss-all-alerts';
    ...
    public function run(Process $process): void
    {
        ...
        $process->setMessages(['LBL_ALERT_DISMISS_ALL_SUCCESS']);
        if (!$result) {
    ...
    protected function dismissAllAlerts(Process $process): bool
    {
        $options = $process->getOptions();

        return $this->alertProvider->dismissAllAlerts(
            $options['ids'] ?? []
        );
    }
}
