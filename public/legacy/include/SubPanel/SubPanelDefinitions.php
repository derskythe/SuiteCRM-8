...
class aSubPanel
{
    ...
    public function __construct(
        ...
        if (!empty($collections) && isset($instance_properties['collection_list' ])) {
            foreach ($instance_properties['collection_list' ] as $cname => $value) {
                if (!in_array($value['module'], $collections, true)) {
    ...
    public function isDatasourceFunction()
    {
        ...
        if (!str_contains($subpanelData, 'function')) {
            return false ;
        }
        return true ;
    }
...
class SubPanelDefinitions
{
    ...
    public function get_available_tabs($FromGetModuleSubpanels = false)
    {
        ...
        if (isset($this->layout_defs [ 'subpanel_setup' ])) { // bug 17434 - belts-and-braces - check that we have some subpanels first
            ...
            if (!empty($hidden_panels) && is_array($hidden_panels) && in_array('activities', $hidden_panels, true)) {
            ...
            foreach ($this->layout_defs [ 'subpanel_setup' ] as $key => $values_array) {
                ...
                if ($hidden_panels && is_array($hidden_panels) && (in_array(
                            $module,
                            $hidden_panels,
                            true
                        ) || array_key_exists($module, $hidden_panels))) {
                ...
                if (!isset($values_array [ 'module' ])) {
                    $GLOBALS['log']->debug(
                        "SubPanelDefinitions->get_available_tabs(): no module defined in subpaneldefs for '$key' =>" . var_export(
                            $values_array,
                            true
                ...
                $GLOBALS ['log']->debug(
                    'SubPanelDefinitions->get_available_tabs(): ' . $key . '= ' . ($exempt
                        ? 'exempt '
                        : 'not exempt ' . ($ok ? ' ACL OK'
                            : ''))
                );
    ...
    public function exclude_tab($tab_name)
    {
        ...
        if (! empty($this->_visible_tabs_array)) {
            $key = array_search($tab_name, $this->_visible_tabs_array, true);
            if ($key !== false) {
