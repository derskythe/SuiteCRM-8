...
// Grab file contents
        ...
        if ($moduleDir === 'Notes') {
            ...
            $contents =
                str_replace(
                    "<tr><div id='portal_flag_row' name='portal_flag_row' style='display:none'>",
                    "<div id='portal_flag_row' name='portal_flag_row' style='display:none'>",
                    $contents
                );
        }
                                ...
                                if (count($matches) > 1) {
                                    ...
                                    foreach ($matches as $tag[1]) {
                                        if (preg_match('/^(mod[\.]|app[\.]).*?/i', $tag[1][1])) {
                                            ...
                                            $theField =
                                                preg_match('/_c$/i', $tag[1][1]) ? $tag[1][1] : strtolower($tag[1][1]);
                                            if (!empty($vardefs[$theField])) {
                                                $field =
                                                    str_replace($tag[1][1], '$fields.' . $theField . '.value', $field);
                                            } else {
