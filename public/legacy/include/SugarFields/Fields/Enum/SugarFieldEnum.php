...
class SugarFieldEnum extends SugarFieldBase
{
    public function getDetailViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex)
    {
        if (!empty($vardef['function']['returns']) && $vardef['function']['returns'] === 'html') {
            ...
            return "<span id='{$vardef['name']}'>" . $this->fetch(
                    $this->findTemplate('DetailViewFunction')
                ) . '</span>';
        } else {
    ...
    public function importSanitize(
        ...
        if (isset($app_list_strings[$vardef['options']])
            ...
            if (in_array($value, $app_list_strings[$vardef['options']], true)) {
                $value = array_search($value, $app_list_strings[$vardef['options']], true);
            }
            // Bug 33328 - Check for a matching key in a different case
            elseif (in_array(
                strtolower($value),
                array_keys(array_change_key_case($app_list_strings[$vardef['options']])),
                true
            )) {
            ...
            elseif (in_array(
                strtolower($value),
                array_map('strtolower', $app_list_strings[$vardef['options']]),
                true
            )) {
                foreach ($app_list_strings[$vardef['options']] as $optionkey => $optionvalue) {
