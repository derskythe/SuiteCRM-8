...
class SugarWidgetFieldDateTimecombo extends SugarWidgetFieldDateTime
{
    ...
    public function queryFilterBetween_Datetimes($layout_def)
    {
        ...
        return '(' . $this->_get_column_select($layout_def) . '>=' . $this->reporter->db->convert(
                $this->reporter->db->quoted($begin),
                'datetime'
            ) .
            " AND\n " . $this->_get_column_select($layout_def) . '<=' . $this->reporter->db->convert(
                $this->reporter->db->quoted($end),
                'datetime'
            ) .
            ")\n";
    }
