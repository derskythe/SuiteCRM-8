...
class nusoap_base
{
    ...
    )
    {
        ...
        switch (true) {
            ...
            case is_object($val):
                ...
                if (get_class($val) === 'soapval') {
                    ...
                    foreach (get_object_vars($val) as $k => $v) {
                        $pXml =
                            isset($pXml) ? $pXml . $this->serialize_val($v, $k, false, false, false, false, $use)
            ...
            case (is_array($val) || $type):
                ...
                if ($valueType === 'arraySimple' || str_starts_with($type, 'ArrayOf')) {
                    ...
                    if (is_array($val) && count($val) > 0) {
                        ...
                        if ($use === 'literal') {
                            ...
                            $type_str =
                                ' xsi:type="SOAP-ENC:Array" SOAP-ENC:arrayType="' . $array_typename . "[$array_type]\"";
                        }
                        // empty array
                    } else {
                        if ($use === 'literal') {
    ...
    )
    {
        ...
        $this->debug(
            'In serializeEnvelope length=' . strlen($body) . ' body (max 1000 characters)=' . substr(
                $body,
                0,
                1000
    ...
    public function schemaStartElement($parser, $name, $attrs)
    {
        ...
        switch ($name) {
            ...
            case 'attribute':    // complexType attribute
                ...
                // arrayType attribute
                if (isset($attrs['http://schemas.xmlsoap.org/wsdl/:arrayType']) || $this->getLocalPart(
                        $aname
                    ) === 'arrayType') {
    ...
    public function serializeSchema()
    {
        ...
        foreach ($this->complexTypes as $typeName => $attrs) {
            ...
            if (isset($attrs['restrictionBase']) && $attrs['restrictionBase'] != '') {
                $contentStr =
                    "   <$schemaPrefix:restriction base=\"" . $this->contractQName(
                        $attrs['restrictionBase']
                    ) . "\">\n" . $contentStr . "   </$schemaPrefix:restriction>\n";
                // complex or simple content
                if ((isset($attrs['elements']) && count($attrs['elements']) > 0) || (isset($attrs['attrs']) && count(
                            $attrs['attrs']
                        ) > 0)) {
                    $contentStr =
        ...
        if (isset($this->elements) && count($this->elements) > 0) {
            foreach ($this->elements as $element => $eParts) {
                $xml .= " <$schemaPrefix:element name=\"$element\" type=\"" . $this->contractQName(
                        $eParts['type']
                    ) . "\"/>\n";
            }
        }
        ...
        if (isset($this->attributes) && count($this->attributes) > 0) {
            foreach ($this->attributes as $attr => $aParts) {
                $xml .= " <$schemaPrefix:attribute name=\"$attr\" type=\"" . $this->contractQName(
                        $aParts['type']
                    ) . "\"\n/>";
            }
        }
    ...
    public function getTypeDef($type)
    {
        ...
        if ((!$is_element) && isset($this->complexTypes[$type])) {
            ...
            if (!isset($this->simpleTypes[$type]['phpType'])) {
                ...
                $uqType =
                    substr($this->simpleTypes[$type]['type'], strrpos($this->simpleTypes[$type]['type'], ':') + 1);
        ...
        } elseif (str_ends_with($type, '_ContainedType')) {
...
class soap_transport_http extends nusoap_base
{
    ...
    public function getResponse()
    {
        ...
        if ($this->io_method() === 'socket') {
            ...
            if (isset($this->incoming_headers['transfer-encoding']) && strtolower(
                    $this->incoming_headers['transfer-encoding']
                ) === 'chunked') {
            ...
            // close filepointer
            if (
                (isset($this->incoming_headers['connection']) && strtolower(
                        $this->incoming_headers['connection']
                    ) === 'close') ||
...
//        }
        ...
        if (isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != '') {
            if (strtolower($this->incoming_headers['content-encoding']) === 'deflate' || strtolower(
...
class nusoap_server extends nusoap_base
{
    ...
    {
        $this->debug(
            'in invoke_method, methodname=' . $this->methodname . ' methodURI=' . $this->methodURI . ' SOAPAction=' . $this->SOAPAction
        );
        ...
        if ($this->wsdl) {
            if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
                ...
                $this->debug(
                    'in invoke_method, found WSDL soapAction=' . $this->SOAPAction . ' for operation=' . $this->opData['name']
                );
                $this->appendDebug('opData=' . $this->varDump($this->opData));
        ...
        if ($class == '') {
            ...
            $method_to_compare = (str_starts_with(phpversion(), '4.')) ? strtolower($method) : $method;
        ...
        if (!function_exists('call_user_func_array')) {
            ...
            if ($class == '') {
                ...
                $call_arg =
                    (string) $this->methodname;    // straight assignment changes $this->methodname to lower case after call_user_func_array()
            } elseif ($delim === '..') {
    ...
    public function serialize_return()
    {
        ...
        // if fault
        if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class(
                        $this->methodreturn
        ...
        if ($this->wsdl) {
            ...
            if ($this->opData['style'] === 'rpc') {
                ...
                if ($this->opData['output']['use'] === 'literal') {
                    ...
                    if ($this->methodURI) {
                        $payload =
                            '<ns1:' . $this->methodname . 'Response xmlns:ns1="' . $this->methodURI . '">' . $return_val . '</ns1:' . $this->methodname . 'Response>';
                    } else {
                        $payload =
...
class wsdl extends nusoap_base
{
    ...
    public function parseWSDL($wsdl = '')
    {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] === 'http' || $wsdl_props['scheme'] === 'https')) {
            ...
            if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] === 'file') && isset($wsdl_props['path'])) {
                $path =
                    isset($wsdl_props['host']) ? ($wsdl_props['host'] . ':' . $wsdl_props['path'])
                        : $wsdl_props['path'];
            } else {
                $path = $wsdl;
            }
                ...
                case 'import':
                    if (isset($attrs['location'])) {
                        $this->import[$attrs['namespace']][] =
                            array( 'location' => $attrs['location'], 'loaded' => false );
                        $this->debug(
     ...
     *                           (optional), verifypeer (optional), verifyhost (optional): see corresponding options in
     *                           cURL docs)
     *
     * @access   public
     */
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    if (isset($opParts['documentation']) && $opParts['documentation'] != '') {
                        $portType_xml .= "\n" . '    <documentation>' . htmlspecialchars(
        ...
        if ($phpType === 'struct') {
            ...
            if (is_null($value)) {
                if ($use === 'literal') {
                    ...
                    $xml =
                        "<$elementName$elementNS xsi:nil=\"true\" xsi:type=\"" . $this->getPrefixFromNamespace(
                            $ns
                        ) . ":$uqType\"/>";
                }
                $this->debug("in serializeType: returning: $xml");
            ...
            if (is_array($value)) {
                ...
                if ($use === 'literal') {
                    if ($forceType) {
                        $xml =
                            "<$elementName$elementNS$elementAttrs xsi:type=\"" . $this->getPrefixFromNamespace(
                                $ns
                            ) . ":$uqType\">";
                    } else {
    ...
    public function __construct($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true)
    {
        ...
        if (!empty($xml)) {
            ...
            if ($pos_xml !== false) {
                ...
                if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res)) {
                    ...
                    if (strtoupper($xml_encoding) != $encoding) {
                        $err =
                            "Charset from HTTP Content-Type '" . $encoding . "' does not match encoding from XML declaration '" . $xml_encoding . "'";
                        $this->debug($err);
            ...
            if (!xml_parse($this->parser, $xml, true)) {
                ...
                $this->debug(
                    'parsed successfully, found root struct: ' . $this->root_struct . ' of name ' . $this->root_struct_name
                );
                // get final value
    ...
    public function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30)
    {
        ...
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                $this->debug(
                    'got response, length=' . strlen(
                        $this->responseData
                    ) . ' type=' . $http->incoming_headers['content-type']
                );
