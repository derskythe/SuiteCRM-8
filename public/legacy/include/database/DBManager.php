...
abstract class DBManager
{
    ...
    protected function registerError(string $userMessage, string $message, bool $dieOnError = false): void
    {
        ...
        if ($dieOnError || $this->dieOnError) {
            if (isset($GLOBALS['app_strings']['ERR_DB_FAIL'])) {
                sugar_die(sprintf('%s%s%s%s%sd',
                    $GLOBALS['app_strings']['ERR_DB_FAIL'], PHP_EOL,
                    $message, PHP_EOL,
                    $this->lastError()));
            } else {
                sugar_die(sprintf('Database error. Please check suitecrm.log for details.%sError: %s',
                    ...
                    if ($field_definition['name'] === 'deleted') {
                        ...
                        if ($this->dbType === 'mysql' && $val === ''
                            ...
                            if (isset($type) && $type === 'int') {
                                ...
                                if (!empty($custom_fields[$field_definition['name']])) {
                                    $cstm_values[$field_definition['name']] =
                                        '\'' . (DBManagerFactory::getInstance() !== null
                                            ? DBManagerFactory::getInstance()->quote(from_html($val))
                                            : null) . '\'';
                                } else {
                                    $values[$field_definition['name']] =
                                        '\'' . (DBManagerFactory::getInstance() !== null
                                            ? DBManagerFactory::getInstance()->quote(from_html($val))
                                            : null) . '\'';
                                }
    ...
    public function updateSQL(SugarBean $bean, array $where = array()): string
    {
        ...
        if (!is_array($fields) && !is_object($fields)) {
            ...
            foreach ((array) $fields as $field_key => $field_definition) {
                ...
                if (!empty($val) && !empty($field_definition['len']) && strlen(
                        (string) $val
                    ) > $field_definition['len']) {
                    $val = $this->truncate($val, $field_definition['len']);
                }

                if (!empty($bean->bean_fields_to_save) && !in_array(
                        $field_definition['name'],
                        $bean->bean_fields_to_save,
                        true
                    )) {
                    continue;
                }
        ...
        if (isset($this->type_class[$type])) {
            ...
            switch ($this->type_class[$type]) {
                ...
                case 'int':
                    ...
                    return $val === '' ? 'NULL'
                        : (int) $val; // Fix #9440 - Forcing default null value for numeric fields.
                case 'bigint':
                    $val = (float) $val;
    ...
    public function massageFieldDef(array &$field_definition, string $table_name) : array
    {
        ...
        if (!empty($field_definition['precision']) && is_numeric($field_definition['precision']) && !str_contains(
                (string) $field_definition['len'],
                ','
            )) {
    ...
    protected function getFieldNameFromSelect(string $string) : string
    {
        ...
        if (str_contains($string, '.')) { //No alias, but a table.field format was used
    ...
    ) : string|array
    {
            ...
            if (in_array($column_base_type, array(
                ...
                if (!empty($field_definition['precision']) && is_numeric($field_definition['precision'])) {
                    if (!str_contains((string) $field_definition['len'], ',')) {
                        $column_type =
                            $column_base_type . '(' . $field_definition['len'] . ',' . $field_definition['precision'] . ')';
                    } else {
                        $column_type = $column_base_type . '(' . $field_definition['len'] . ')';
                    }
                } else {
        ...
        if (isset($field_definition['isnull']) && (strtolower(
                    $field_definition['isnull']
                ) === 'false' || $field_definition['isnull'] === false)
     ...
     */
    public function getValidDBName(
        array|string $name,
        bool|string  $ensureUnique = false,
        string       $type = 'column',
        bool         $force = false
    ) : array|string
    {
    ...
    protected function auditSQL(SugarBean $bean, array $changes) : string
    {
        ...
        if ($changes['data_type'] === 'text' || $changes['data_type'] === 'multienum') {
            $values['before_value_text'] =
                $this->massageValue($changes['before'], $field_definitions['before_value_text']);
            $values['after_value_text'] =
                $this->massageValue($changes['after'], $field_definitions['after_value_text']);
        } else {
            $values['before_value_string'] =
                $this->massageValue($changes['before'], $field_definitions['before_value_string']);
            $values['after_value_string'] =
                $this->massageValue($changes['after'], $field_definitions['after_value_string']);
        }
        $values['date_created'] =
            $this->massageValue(TimeDate::getInstance()->nowDb(), $field_definitions['date_created']);
    ...
    public function checkPrivilege(string $privilege) : bool
    {
        switch ($privilege) {
            ...
            case 'UPDATE':
                $this->query(
                    'UPDATE temp SET id = \'100000000000000000000000000000000000\' WHERE id = \'abcdef0123456789abcdef0123456789abcd\''
                );
                break;
            case 'SELECT':
                return !empty($this->getOne('SELECT id FROM temp WHERE id=\'100000000000000000000000000000000000\'', false));
            case 'DELETE':
     ...
     * @return mixed
     */
    public function canInstall() : array|bool
    {
        return true;
    }
     ...
     */
    abstract public function limitQuery(string $sql, int $start, int $count, bool $dieOnError = false,
                                        string                                    $msg = '',
                                        bool                                      $execute = true
    );
     ...
     */
    abstract public function renameColumnSQL(string $table_name, string $column, string $new_name) : string;
     ...
     * @return string SQL Create Table statement
     */
    abstract public function createTableSQLParams(string  $table_name,
     ...
     */
    abstract public function getFulltextQuery(string $field,
                                              array  $terms,
     ...
     * @return array
     */
    abstract public function installConfig(): array;
