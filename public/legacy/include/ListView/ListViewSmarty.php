...
class ListViewSmarty extends ListViewDisplay
{
    ...
    public bool $showMassupdateFields = true;
    public string $menu_location = 'top';
    public array $templateMeta = array();
    public bool $displayEmptyDataMessages = false;
     ...
     */
    public function buildSendConfirmOptInEmailToPersonAndCompany() : string
    {
    ...
    public function process(string $file, mixed $data, string $htmlVar) : bool
    {
        ...
        foreach ((array) $this->displayColumns as $name => $params) {
            if (!isset($this->displayColumns[$name]['width']) || 0 === $adjustment) {
                ...
                $this->displayColumns[$name]['width'] =
                    floor(((int) $this->displayColumns[$name]['width']) / $adjustment);
            }
        ...
        $this->ss->assign('options', $this->templateMeta['options'] ?? null);
        $this->ss->assign('form', $this->templateMeta['form'] ?? null);
        $this->ss->assign('includes', $this->templateMeta['includes'] ?? null);
        ...
        $this->ss->assign('htmlpublic', strtoupper($htmlVar));
        ...
        if (!isset($this->data['pageData']['offsets'])) {
            ...
            if ($this->data['pageData']['offsets']['next'] < 0) {
                $pageTotal =
                    $this->data['pageData']['offsets']['total'] - $this->data['pageData']['offsets']['current'];
            }

            if ($this->select) {
                $this->ss->assign(
                    'selectLinkTop',
                    $this->buildSelectLink(
                        'select_link',
                        $this->data['pageData']['offsets']['total'],
                        $pageTotal
                    )
                );
            }
        ...
        if (isset($_REQUEST['module']) && in_array(
                $_REQUEST['module'],
        ...
        $this->ss->assign('showFilterIcon', !in_array($module, $sugar_config['enable_legacy_search'] ?? array(), true));
     ...
     */
    public function processArrows(string $ordering = '') : void
    {
        $pathParts = pathinfo(SugarThemeRegistry::current()->getImageURL('arrow.gif', false));

        [ $width, $height ] = getimagesize($pathParts['dirname'] . DIRECTORY_SEPARATOR . $pathParts['basename']);
     ...
     * @throws SmartyException
     */
    public function display(bool $end = true) : string
    {
        if (!$this->should_process) {
        ...
        if (!isset($this->data['pageData']['offsets'])) {
            ...
            if (!isset($this->data['data'])) {
                ...
                LoggerManager::getLogger()?->warn(
                    'List view smarty data must be an array, undefined data given and converting to an empty array.'
                );
            } elseif (!is_array($this->data['data'])) {
                LoggerManager::getLogger()?->warn(
                    'List view smarty data must be an array, ' . gettype(
                        $this->data['data']
                    ) . ' given and converting to an array.'
                );
            }
            $this->data['pageData']['offsets']['lastOffsetOnPage'] =
                $this->data['pageData']['offsets']['current'] + count((array) $this->data['data']);
        }
        ...
        $displayEmptyDataMessages = $this->displayEmptyDataMessages ?? true;
        ...
        if ((isset($GLOBALS['moduleTabMap'][$currentModule]) && $GLOBALS['moduleTabMap'][$currentModule] === 'Administration')
            || isset($GLOBALS['adminOnlyList'][$currentModule]) || in_array(
                $currentModule,
     ...
     * @throws SmartyException
     */
    private function getSearchIcon() : string
    {
        global $sugar_config;

        $searchFormInPopup = !in_array(
            $_REQUEST['module'],
