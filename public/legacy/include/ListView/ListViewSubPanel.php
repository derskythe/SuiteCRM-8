                ...
                if ($usage === 'query_only' && !empty($list_field['force_query_only_display'])) {
                    ...
                    if ($usage !== 'query_only') {
                        ...
                        if (isset($list_field['alias'])) {
                            ...
                            if (!isset($fields[strtoupper($list_field['alias'])]) || empty(
                                $fields[strtoupper(
                                    $list_field['alias']
                                )]
                                )) {
                                global $timedate;
                                $fields[strtoupper($list_field['alias'])] =
                                    (!empty($aItem->$field_name)) ? $aItem->$field_name
                                        : $timedate->to_display_date_time($aItem->{$list_field['alias']});
                            }
                        } else {
                            $list_field['name'] = $field_name;
                        }
                        ...
                        if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                            ...
                            if (isset($list_field['widget_class']) && $list_field['widget_class'] === 'SubPanelDetailViewLink') {
                                ...
                                if ('full_name' === $field_name) {//bug #32465
                                    $list_field['fields'][strtoupper($field_name)] =
                                        $widget_contents[$aVal][$field_name];
                                }
                                ...
                                if (isset($list_field['widget_class']) && $list_field['widget_class'] === 'SubPanelEmailLink') {
                                    if (isset($list_field['fields']['EMAIL1_LINK'])) {
                                        ...
                                        $widget_contents[$aVal][$field_name] =
                                            $layout_manager->widgetDisplay($list_field);
                                    }
                                }
                            }
                            ...
                            if (isset($list_field['widget_class']) && $list_field['widget_class'] === 'SubPanelIcon') {
                                ...
                                if ((($list_field['name'] === 'edit_button' && $field_acl['EditView']) || ($list_field['name'] === 'close_button' && $field_acl['EditView']) || ($list_field['name'] === 'remove_button' && $field_acl['EditView'])) && '' != (
                                    $_content = $layout_manager->widgetDisplay($list_field))) {
                                    $button_contents[$aVal][] = $_content;
                                    unset($_content);
                                } else {
                                    $doNotProcessTheseActions = array( 'edit_button',
                                                                       'close_button',
                                                                       'remove_button' );
                                    if (!in_array($list_field['name'], $doNotProcessTheseActions, true) && '' != (
                                        $_content = $layout_manager->widgetDisplay($list_field))) {
                                        $button_contents[$aVal][] = $_content;
                                        unset($_content);
                                    } else {
...
//                                    $this->smartyTemplate->assign('CELL_COUNT', $count);
    ...
    )
    {
        ...
        if (empty($this->query_limit) || $this->query_limit > $this->records_per_page) {
            ...
            if (0 == $current_offset) {
                $start_link =
                    "<button type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' class='button' disabled><img src='" . SugarThemeRegistry::current(
                    )->getImageURL('paginate_first.svg') . "'/></button>";
                $previous_link =
                    "<button type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' class='button' disabled><img src='" . SugarThemeRegistry::current(
                    )->getImageURL('paginate_previous.svg') . "'/></button>";
            } else {
                if ($this->multi_select_popup) {// nav links for multiselect popup, submit form to save checks.
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='javascript:save_checks(0, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-first'></span></button>";
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='javascript:save_checks($previous_offset, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-left'></span></button>";
                } elseif ($this->shouldProcess) {
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='location.href=\"$start_URL\"; sListView.save_checks(0, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-first'></span></button>";
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='location.href=\"$previous_URL\"; sListView.save_checks($previous_offset, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-left'></span></button>";
                } else {
                    $onClick = '';
                    ...
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick=" . $onClick . "><span class='suitepicon suitepicon-action-first'></span></button>";
                    ...
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick=" . $onClick . "><span class='suitepicon suitepicon-action-left'></span></button>";
                }
            }

            if ($last_offset <= $current_offset) {
                $end_link =
                    "<button type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' class='button' disabled><img src='" . SugarThemeRegistry::current(
                    )->getImageURL('paginate_last.svg') . "'/></button>";
                $next_link =
                    "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' disabled><img src='" . SugarThemeRegistry::current(
                    )->getImageURL('paginate_next.svg') . "'/></button>";
            } else {
                if ($this->multi_select_popup) { // nav links for multiselect popup, submit form to save checks.
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='javascript:save_checks($last_offset, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-last'></span></button>";
                    if (!empty($sugar_config['disable_count_query'])) {
                        $end_link = '';
                    }
                    $next_link =
                        "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' onClick='javascript:save_checks($next_offset, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-right'></span></button>";
                } elseif ($this->shouldProcess) {
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='location.href=\"$end_URL\"; sListView.save_checks(\"end\", \"{$moduleString}\");'><span class='suitepicon suitepicon-action-last'></span></button>";
                    $next_link =
                        "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick='location.href=\"$next_URL\"; sListView.save_checks($next_offset, \"{$moduleString}\");'><span class='suitepicon suitepicon-action-right'></span></button>";
                } else {
                    $onClick = '';
                    ...
                    $next_link =
                        "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick=" . $onClick . "><span class='suitepicon suitepicon-action-right'></span></button>";
                    ...
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick=" . $onClick . "><span class='suitepicon suitepicon-action-last'></span></button>";
                }
            }
            ...
            $close_inline_img = SugarThemeRegistry::current()->getImage(
                'close_inline',
                'border=0',
                null,
                null,
            ...
            $export_link =
                '<input class="button" type="button" value="' . $this->local_app_strings['LBL_EXPORT'] . '" ' . 'onclick="return sListView.send_form(true, \'' . $_REQUEST['module'] . '\', \'index.php?entryPoint=export\',\'' . $this->local_app_strings['LBL_LISTVIEW_NO_SELECTED'] . '\')">';
            ...
            $selected_objects_span =
                "&nbsp;|&nbsp;{$this->local_app_strings['LBL_LISTVIEW_SELECTED_OBJECTS']}<input  style='border: 0px; background: transparent; font-size: inherit; color: inherit' type='text' readonly name='selectCount[]' value='" . ((isset($_POST['mass']))
                    ? count($_POST['mass']) : 0) . "' />";

            if ($_REQUEST['module'] === 'Home' || $this->local_current_module === 'Import' || $this->show_export_button == false || (!empty($sugar_config['disable_export'])) || (!empty($sugar_config['admin_export_only']) && !(is_admin(
                            $current_user
                        ) || (ACLController::moduleSupportsACL($_REQUEST['module']) && ACLAction::getUserAccessLevel(
                                $current_user->id,
                                $_REQUEST['module'],
                                'access'
                            ) == ACL_ALLOW_ENABLED && (ACLAction::getUserAccessLevel(
                                    $current_user->id,
                                    $_REQUEST['module'],
                                    'admin'
                                ) == ACL_ALLOW_ADMIN || ACLAction::getUserAccessLevel(
                                    $current_user->id,
                                    $_REQUEST['module'],
                                    'admin'
                                ) == ACL_ALLOW_ADMIN_DEV))))) {
