...
class ListView
{
    ...
    public function processListViewMulti($seed, $xTemplateSection, $html_varName)
    {
        ...
        echo "<a href='javascript:" . ((!$this->multi_select_popup) ? 'sListView.'
                : '') . "check_all(document.MassUpdate, \"mass[]\", true)'>" . translate(
                'LBL_CHECKALL'
            ) . "</a> - <a href='javascript:sListView.check_all(document.MassUpdate, \"mass[]\", false);'>" . translate(
                'LBL_CLEARALL'
            ) . '</a>';
        echo '<br><br>';
    }
    ...
    public function processListView($seed, $xTemplateSection, $html_varName)
    {
        ...
        $populateOnly =
            $this->ignorePopulateOnly ? false
                : (!empty($sugar_config['save_query']) && $sugar_config['save_query'] === 'populate_only');
        if (isset($seed->module_dir) && $populateOnly) {
            if (empty($GLOBALS['displayListView']) && strcmp(
                    strtolower($_REQUEST['action']),
                    'popup'
                ) != 0 && (!empty($_REQUEST['clear_query']) || $_REQUEST['module'] == $seed->module_dir && ((empty($_REQUEST['query']) || $_REQUEST['query'] === 'MSI') && (empty($_SESSION['last_search_mod']) || $_SESSION['last_search_mod'] != $seed->module_dir)))) {
        ...
        if (!isset($_REQUEST['action'])) {
            ...
            $this->shouldProcess = is_subclass_of($seed, 'SugarBean')
                && (($_REQUEST['action'] === 'index') || (str_starts_with($_REQUEST['action'], 'ListView')) /* cn: to include all ListViewXXX.php type views */)
    ...
    public function process_dynamic_listview($source_module, $sugarbean, $subpanel_def)
    {
        ...
        if (isset($_SESSION['validation'])) {
            print base64_decode(
                'PGEgaHJlZj0naHR0cDovL3d3dy5zdWdhcmNybS5jb20nPlBPV0VSRUQmbmJzcDtCWSZuYnNwO1NVR0FSQ1JNPC9hPg=='
            );
        }
                ...
                if ($usage === 'query_only' && !empty($list_field['force_query_only_display'])) {
                    ...
                    if ($usage !== 'query_only') {
                        ...
                        if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                            ...
                            if ($tmpField != null) {
                                $widget_contents =
                                    SugarFieldHandler::displaySmarty(
                                        $list_field['fields'],
                                        $vardef,
                                        'ListView',
                                        $list_field
                            ...
                            if (isset($list_field['widget_class']) && $list_field['widget_class'] === 'SubPanelIcon') {
                                ...
                                if ((($list_field['name'] === 'edit_button' && $field_acl['EditView']) || ($list_field['name'] === 'close_button' && $field_acl['EditView']) || ($list_field['name'] === 'remove_button' && $field_acl['Delete'])) && '' != (
    ...
    /**
     * INTERNAL FUNCTION returns the offset first checking the query then checking the session if the where clause has
     * changed from the last time it returns 0
     *
     * @param $localVarName
     *
     * @return int
    ...
    public function processSugarBean($xtemplateSection, $html_varName, $seed)
    {
        ...
        $main_query =
            $ret_array['select'] . $params['custom_select'] . $ret_array['from'] . $params['custom_from'] . $ret_array['where'] . $params['custom_where'] . $ret_array['order_by'] . $params['custom_order_by'];
        ...
        $this->processListNavigation(
            ...
            null,
            null,
            empty($seed->column_fields) ? null : count($seed->column_fields)
        );
    ...
    public function getBaseURL($html_varName)
    {
        ...
        $blockVariables =
            array( 'mass', 'uid', 'massupdate', 'delete', 'merge', 'selectCount', 'current_query_by_page' );
        ...
        foreach (array_merge($_GET, $_POST) as $name => $value) {
            ...
            if (!empty($value) && $name !== 'sort_order' //&& $name != ListView::getSessionVariableName($html_varName,"ORDER_BY")
                ...
                /*&& substr_count($name, "ORDER_BY")==0*/ && !in_array($name, $blockVariables, true)) {
                if (is_array($value)) {
    ...
    )
    {
        ...
        if (empty($this->query_limit) || $this->query_limit > $this->records_per_page) {
            ...
            if ($this->is_dynamic) {
                $dynamic_url .= '&' . $this->getSessionVariableName(
                        $html_varName,
                        'ORDER_BY'
                    ) . '=' . $this->getSessionVariable(
                        $html_varName,
                        'ORDER_BY'
                    ) . '&sort_order=' . $this->sort_order . '&to_pdf=true&action=SubPanelViewer&subpanel=' . $this->subpanel_module;
            }
            ...
            if ($this->shouldProcess && !$this->multi_select_popup) {
                ...
                $select_entire_list =
                    ($massUpdateRun)
                        ? 0
                        : (isset($_POST['select_entire_list'])
                        ? $_POST['select_entire_list']
                        : (isset($_REQUEST['select_entire_list']) ? htmlspecialchars($_REQUEST['select_entire_list'])
                            : 0));
            ...
            $GLOBALS['log']->debug(
                "Offsets: (start, previous, next, last)(0, $previous_offset, $next_offset, $last_offset)"
            );

            if (0 == $current_offset) {
                $start_link =
                    "<button type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' class='button' disabled>" . SugarThemeRegistry::current(
                    )->getImage(
                        'start_off',
                        "aborder='0' align='absmiddle'",
                        null,
                        null,
                        '.gif',
                        $this->local_app_strings['LNK_LIST_START']
                    ) . '</button>';
                $previous_link =
                    "<button type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' class='button' disabled>" . SugarThemeRegistry::current(
                    )->getImage(
                        'previous_off',
                        ...
                        null,
                        '.gif',
                        $this->local_app_strings['LNK_LIST_PREVIOUS']
                    ) . '</button>';
            } else {
                if ($this->multi_select_popup) {// nav links for multiselect popup, submit form to save checks.
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='javascript:save_checks(0, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'start',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='javascript:save_checks($previous_offset, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'previous',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='location.href=\"$start_URL\"; sListView.save_checks(0, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'start',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='location.href=\"$previous_URL\"; sListView.save_checks($previous_offset, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'previous',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $start_link =
                        "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick=" . $onClick . '>' . SugarThemeRegistry::current(
                        )->getImage(
                            'start',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $previous_link =
                        "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick=" . $onClick . '>' . SugarThemeRegistry::current(
                        )->getImage(
                            'previous',
                            "border='0' align='absmiddle'",
                            null,
            ...
            if ($last_offset <= $current_offset) {
                $end_link =
                    "<button type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' class='button' disabled>" . SugarThemeRegistry::current(
                    )->getImage(
                        'end_off',
                        "border='0' align='absmiddle'",
                        null,
                        null,
                        '.gif',
                        $this->local_app_strings['LNK_LIST_END']
                    ) . '</button>';
                $next_link =
                    "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' disabled>" . SugarThemeRegistry::current(
                    )->getImage(
                        'next_off',
                        "aborder='0' align='absmiddle'",
                        null,
                        null,
                        '.gif',
                        $this->local_app_strings['LNK_LIST_NEXT']
                    ) . '</button>';
            } else {
                if ($this->multi_select_popup) { // nav links for multiselect popup, submit form to save checks.
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='javascript:save_checks($last_offset, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'end',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $next_link =
                        "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' onClick='javascript:save_checks($next_offset, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'next',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='location.href=\"$end_URL\"; sListView.save_checks(\"end\", \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'end',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $next_link =
                        "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick='location.href=\"$next_URL\"; sListView.save_checks($next_offset, \"{$moduleString}\");'>" . SugarThemeRegistry::current(
                        )->getImage(
                            'next',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $next_link =
                        "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick=" . $onClick . '>' . SugarThemeRegistry::current(
                        )->getImage(
                            'next',
                            "border='0' align='absmiddle'",
                            null,
                    ...
                    $end_link =
                        "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick=" . $onClick . '>' . SugarThemeRegistry::current(
                        )->getImage(
                            'end',
                            "border='0' align='absmiddle'",
                            null,
            ...
            $export_link =
                '<input class="button" type="button" value="' . $this->local_app_strings['LBL_EXPORT'] . '" ' .
            ...
            if ($this->show_delete_button) {
                $delete_link =
                    '<input class="button" type="button" id="delete_button" name="Delete" value="' . $this->local_app_strings['LBL_DELETE_BUTTON_LABEL'] . '" onclick="return sListView.send_mass_update(\'selected\',\'' . $this->local_app_strings['LBL_LISTVIEW_NO_SELECTED'] . '\', 1)">';
            } else {
                $delete_link = '&nbsp;';
            }
            ...
            if ($user_merge === 'on' && isset($admin->settings['system_mailmerge_on']) && $admin->settings['system_mailmerge_on']) {
                ...
                $merge_link =
                    "&nbsp;|&nbsp;<a id='mailmerge_link' onclick='return mailmerge_dialog(this)'; href=\"javascript:void(0)\">" . $this->local_app_strings['LBL_MAILMERGE'] . '</a>';
            } else {
                $merge_link = '&nbsp;';
            }

            $selected_objects_span =
                "&nbsp;&nbsp;{$this->local_app_strings['LBL_LISTVIEW_SELECTED_OBJECTS']}<input class='listviewcount' type='text' readonly name='selectCount[]' value='" . ((isset($_POST['mass']))
                    ? count($_POST['mass']) : 0) . "' />";
            ...
            if ($this->show_paging == true) {
                ...
                if ($subpanel_def != null) {
                    ...
                    if (!empty($this->response)) {
                        ...
                        $response =
                            SugarBean::get_union_related_list(
                                $sugarbean,
                                $this->sortby,
                                $this->sort_order,
                                $this->query_where,
        ...
        if (!isset($this->base_URL)) {
            ...
            if (isset($_SERVER['QUERY_STRING'])) {
                $this->base_URL = preg_replace(
                    '/\&' . $this->getSessionVariableName($html_varName, 'ORDER_BY') . '=[0-9a-zA-Z\_\.]*/',
                    '',
                    $this->base_URL . '?' . $_SERVER['QUERY_STRING']
                );
                $this->base_URL = preg_replace(
                    '/\&' . $this->getSessionVariableName($html_varName, 'offset') . '=[0-9]*/',
                    '',
                    $this->base_URL
                );
            }
    ...
    public function processListRows($data, $xtemplateSection, $html_varName)
    {
        ...
        foreach ($data as $aVal => $aItem) {
            ...
            if ($this->shouldProcess) {
                ...
                $this->xTemplate->assign(
                    'CHECKALL',
                    "<label class=\"hidden glyphicon bootstrap-checkbox glyphicon-unchecked\"></label><span class='suitepicon suitepicon-action-caret'></span><input type='checkbox' class='bootstrap-checkbox-hidden checkbox'  title='" . $GLOBALS['app_strings']['LBL_SELECT_ALL_TITLE'] . "'  name='massall' id='massall' value='' onclick='sListView.check_all(document.MassUpdate, \"mass[]\", this.checked)'>"
                );
            }
            if (!isset($this->data_array)) {
            ...
            if (isset($this->data_array)) {
                ...
                // Bug #2786
                if ($this->_additionalDetails && $aItem->ACLAccess('DetailView') && (file_exists(
                            'modules/' . $aItem->module_dir . '/metadata/additionalDetails.php'
                        ) || file_exists('custom/modules/' . $aItem->module_dir . '/metadata/additionalDetails.php'))) {
                    ...
                    if (file_exists('custom/modules/' . $aItem->module_dir . '/metadata/additionalDetails.php')) {
                        $additionalDetailsFile =
                            'custom/modules/' . $aItem->module_dir . '/metadata/additionalDetails.php';
                    }

                    require_once($additionalDetailsFile);
                    $ad_function =
                        (empty($this->additionalDetailsFunction) ? 'additionalDetails'
                            : $this->additionalDetailsFunction) . $aItem->object_name;
                ...
                if ($aItem->ACLAccess('Delete')) {
                    $delete =
                        '<a class="listViewTdToolsS1" onclick="return confirm(\'' . $this->local_app_strings['NTC_DELETE_CONFIRMATION'] . '\')" href="' . 'index.php?action=Delete&module=' . $aItem->module_dir . '&record=' . $fields['ID'] . '&return_module=' . $aItem->module_dir . '&return_action=index&return_id=">' . $this->local_app_strings['LBL_DELETE_INLINE'] . '</a>';
                    require_once('include/Smarty/plugins/function.sugar_action_menu.php');
    ...
    public function process_dynamic_listview_header($source_module, $subpanel_def, $html_var = 'CELL')
    {
        ...
        foreach ($subpanel_def->get_list_fields() as $column_name => $widget_args) {
            ...
            if ($usage !== 'query_only' || !empty($widget_args['force_query_only_display'])) {
                ...
                if ($orderBy == $column_name || (isset($widget_args['sort_by']) && str_replace(
                            '.',
                            '_',
                            $widget_args['sort_by']
                        ) == $orderBy)) {
     ...
     * $XTemplateSection is the section in the XTemplate file that should be parsed usually main
     * $html_VarName is the variable name used in the XTemplateFile e.g. TASK
     * $seed is a seed there are two types of seeds one is a subclass of SugarBean, the other is a list usually created
     * from a sugar bean using get_list if no XTemplate is set it will create  a new XTemplate Portions created by
     * SugarCRM are Copyright (C) SugarCRM, Inc.. All Rights Reserved.. Contributor(s):
