 ...
 */
 ...
 class pDraw
 {
     ...
     public function drawPolygon($Points, $Format= '')
     {
         ...
         if (!$NoFill) {
             if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
                 ...
                 $this->drawPolygon($Shadow, array( 'R'        =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB, 'Alpha' =>$this->Shadowa,
                                                    'NoBorder' =>true));
             }
     ...
     public function drawRoundedFilledRectangle($X1, $Y1, $X2, $Y2, $Radius, $Format= '')
     {
         ...
         if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
             $this->Shadow = false;
             $this->drawRoundedFilledRectangle($X1+$this->ShadowX, $Y1+$this->ShadowY, $X2+$this->ShadowX, $Y2+$this->ShadowY, $Radius, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB,
                                                                                                                                               'Alpha' =>$this->Shadowa));
         }
         ...
         $Radius++;
         $this->drawRoundedRectangle($X1, $Y1, $X2+1, $Y2-1, $Radius, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                             'Alpha' =>$Alpha));
     ...
     public function drawRoundedFilledRectangle_deprecated($X1, $Y1, $X2, $Y2, $Radius, $Format= '')
     {
         ...
         $this->drawFilledRectangle($X1-$XOffset1, $YTop, $X2+$XOffset2, $YBottom, array( 'R'        =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                                          'NoBorder' =>true));
         ...
         $this->drawRoundedRectangle($X1, $Y1, $X2, $Y2, $Radius, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                         'Alpha' =>$Alpha));
     ...
     public function drawRectangle($X1, $Y1, $X2, $Y2, $Format= '')
     {
         ...
         if ($this->Antialias) {
             if ($NoAngle) {
                 $this->drawLine($X1+1, $Y1, $X2-1, $Y1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
                 $this->drawLine($X2, $Y1+1, $X2, $Y2-1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
                 $this->drawLine($X2-1, $Y2, $X1+1, $Y2, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
                 $this->drawLine($X1, $Y1+1, $X1, $Y2-1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
             } else {
                 $this->drawLine($X1+1, $Y1, $X2-1, $Y1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
     ...
     public function drawFilledRectangle($X1, $Y1, $X2, $Y2, $Format= '')
     {
         ...
         if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
             $this->Shadow = false;
             $this->drawFilledRectangle($X1+$this->ShadowX, $Y1+$this->ShadowY, $X2+$this->ShadowX, $Y2+$this->ShadowY, array( 'R'       =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB, 'Alpha' =>$this->Shadowa, 'Ticks' =>$Ticks,
                                                                                                                               'NoAngle' =>$NoAngle));
         }
         ...
         if ($BorderR != -1) {
             $this->drawRectangle($X1, $Y1, $X2, $Y2, array( 'R'       =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha, 'Ticks' =>$Ticks,
                                                             'NoAngle' =>$NoAngle));
         }
     ...
     public function drawBezier($X1, $Y1, $X2, $Y2, $Xv1, $Yv1, $Xv2, $Yv2, $Format= '')
     {
         ...
         if (!$NoDraw) {
             ...
             if ($ShowC && !$PathOnly) {
                 ...
                 $MyMarkerSettings = array( 'R'    =>255, 'G' =>0, 'B' =>0, 'BorderR' =>255, 'BorderB' =>255, 'BorderG' =>255,
                                            'Size' =>4);
                 $this->drawRectangleMarker($Xv1, $Yv1, $MyMarkerSettings);
                 $this->drawText($Xv1+4, $Yv1, 'v1');
                 $MyMarkerSettings = array( 'R'    =>0, 'G' =>0, 'B' =>255, 'BorderR' =>255, 'BorderB' =>255, 'BorderG' =>255,
                                            'Size' =>4);
             ...
             foreach ($Q as $Key => $Point) {
                 ...
                 if ($LastX != null && $LastY != null && !$PathOnly) {
                     list($Cpt, $Mode) = $this->drawLine($LastX, $LastY, $X, $Y, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks, 'Cpt' =>$Cpt, 'Mode' =>$Mode,
                                                                                        'Weight' =>$Weight));
                 }
             ...
             if ($DrawArrow && !$PathOnly) {
                 $ArrowSettings = array( 'FillR' =>$R, 'FillG' =>$G, 'FillB' =>$B, 'Alpha' =>$Alpha, 'Size' =>$ArrowSize,
                                         'Ratio' =>$ArrowRatio);
                 if ($ArrowTwoHeads) {
     ...
     public function drawCircle($Xc, $Yc, $Height, $Width, $Format= '')
     {
         ...
         if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
             $this->Shadow = false;
             $this->drawCircle($Xc+$this->ShadowX, $Yc+$this->ShadowY, $Height, $Width, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB, 'Alpha' =>$this->Shadowa,
                                                                                               'Ticks' =>$Ticks));
         }
     ...
     public function drawFilledCircle($X, $Y, $Radius, $Format= '')
     {
         ...
         if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
             $this->Shadow = false;
             $this->drawFilledCircle($X+$this->ShadowX, $Y+$this->ShadowY, $Radius, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB, 'Alpha' =>$this->Shadowa,
                                                                                           'Ticks' =>$Ticks));
         }
         ...
         if ($this->Antialias) {
             $this->drawCircle($X, $Y, $Radius, $Radius, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                'Ticks' =>$Ticks));
         }
         ...
         if ($BorderR != -1) {
             $this->drawCircle($X, $Y, $Radius, $Radius, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                'Ticks' =>$Ticks));
         }
     ...
     public function drawGradientArea($X1, $Y1, $X2, $Y2, $Direction, $Format= '')
     {
         ...
         if ($StartR == $EndR && $StartG == $EndG && $StartB == $EndB) {
             $this->drawFilledRectangle($X1, $Y1, $X2, $Y2, array( 'R'     =>$StartR, 'G' =>$StartG, 'B' =>$StartB,
                                                                   'Alpha' =>$Alpha));
             return(0);
         }
     ...
     public function drawFromPicture($PicType, $FileName, $X, $Y)
     {
         if (file_exists($FileName)) {
             ...
             if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
                 $this->Shadow = false;
                 if ($PicType == 3) {
                     $this->drawFilledRectangle($X+$this->ShadowX, $Y+$this->ShadowY, $X+$Width+$this->ShadowX, $Y+$Height+$this->ShadowY, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB,
                                                                                                                                                  'Alpha' =>$this->Shadowa));
                 } else {
     ...
     public function drawArrow($X1, $Y1, $X2, $Y2, $Format= '')
     {
         ...
         if ($this->Shadow && $this->ShadowX != 0 && $this->ShadowY != 0) {
             $this->Shadow = false;
             $this->drawArrow($X1+$this->ShadowX, $Y1+$this->ShadowY, $X2+$this->ShadowX, $Y2+$this->ShadowY, array( 'FillR' =>$this->ShadowR, 'FillG' =>$this->ShadowG, 'FillB' =>$this->ShadowB, 'Alpha' =>$this->Shadowa, 'Size' =>$Size, 'Ratio' =>$Ratio, 'TwoHeads' =>$TwoHeads,
                                                                                                                     'Ticks' =>$Ticks));
         }
         ...
         $this->drawLine($Points[0], $Points[1], $Points[2], $Points[3], array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                'Alpha' =>$Alpha));
         $this->drawLine($Points[2], $Points[3], $Points[4], $Points[5], array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                'Alpha' =>$Alpha));
         $this->drawLine($Points[0], $Points[1], $Points[4], $Points[5], array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                'Alpha' =>$Alpha));
         ...
         if ($TwoHeads) {
             ...
             $this->drawLine($TailX, $TailY, $TailX2, $TailY2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$Alpha,
                                                                      'Ticks' =>$Ticks));
         } else {
             $this->drawLine($X1, $Y1, $TailX, $TailY, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$Alpha,
                                                              'Ticks' =>$Ticks));
         }
     ...
     public function drawArrowLabel($X1, $Y1, $Text, $Format= '')
     {
         ...
         if ($Angle > 0 && $Angle < 180) {
             $this->drawLine($X2, $Y2, $X2-$TxtWidth, $Y2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$Alpha,
                                                                  'Ticks' =>$Ticks));
             if ($Position == POSITION_TOP) {
                 $this->drawText($X2, $Y2-2, $Text, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$Alpha,
                                                           'Align' =>TEXT_ALIGN_BOTTOMRIGHT));
             } else {
                 $this->drawText($X2, $Y2+4, $Text, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$Alpha,
     ...
     public function drawProgress($X, $Y, $Percent, $Format= '')
     {
         ...
         if ($Orientation == ORIENTATION_VERTICAL) {
             ...
             $this->drawFilledRectangle($X, $Y, $X+$Width, $Y-$Height, array( 'R'       =>$BoxBackR, 'G' =>$BoxBackG, 'B' =>$BoxBackB, 'BorderR' =>$BoxBorderR, 'BorderG' =>$BoxBorderG, 'BorderB' =>$BoxBorderB,
                                                                              'NoAngle' =>$NoAngle));

             $RestoreShadow = $this->Shadow;
             $this->Shadow  = false;
             if ($RFade != -1 && $GFade != -1 && $BFade != -1) {
                 $GradientOptions = array( 'StartR' =>$RFade, 'StartG' =>$GFade, 'StartB' =>$BFade, 'EndR' =>$R, 'EndG' =>$G,
                                           'EndB'   =>$B);
                 $this->drawGradientArea($X+1, $Y-1, $X+$Width-1, $Y-$InnerHeight, DIRECTION_VERTICAL, $GradientOptions);

                 if ($Surrounding) {
                     $this->drawRectangle($X+1, $Y-1, $X+$Width-1, $Y-$InnerHeight, array( 'R'     =>255, 'G' =>255, 'B' =>255,
                                                                                           'Alpha' =>$Surrounding));
                 }
             } else {
                 $this->drawFilledRectangle($X+1, $Y-1, $X+$Width-1, $Y-$InnerHeight, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                                                                             'BorderB' =>$BorderB));
             }
             ...
             if ($ShowLabel && $LabelPos == LABEL_POS_INSIDE) {
                 $this->drawText($X+($Width/2), $Y-$InnerHeight-$Margin, $Percent. '%', array( 'Align' =>TEXT_ALIGN_MIDDLELEFT,
                                                                                               'Angle' =>90));
             }
             ...
             $this->Shadow  = false;
             if ($RFade != -1 && $GFade != -1 && $BFade != -1) {
                 $GradientOptions = array( 'StartR' =>$R, 'StartG' =>$G, 'StartB' =>$B, 'EndR' =>$RFade, 'EndG' =>$GFade,
                                           'EndB'   =>$BFade);
                 $this->drawGradientArea($X+1, $Y+1, $X+$InnerWidth, $Y+$Height-1, DIRECTION_HORIZONTAL, $GradientOptions);

                 if ($Surrounding) {
                     $this->drawRectangle($X+1, $Y+1, $X+$InnerWidth, $Y+$Height-1, array( 'R'     =>255, 'G' =>255, 'B' =>255,
                                                                                           'Alpha' =>$Surrounding));
                 }
             } else {
                 $this->drawFilledRectangle($X+1, $Y+1, $X+$InnerWidth, $Y+$Height-1, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                                                                             'BorderB' =>$BorderB));
             }
     ...
     public function drawLegend($X, $Y, $Format= '')
     {
         ...
         if ($Style == LEGEND_ROUND) {
             $this->drawRoundedFilledRectangle($Boundaries['L']-$Margin, $Boundaries['T']-$Margin, $Boundaries['R']+$Margin, $Boundaries['B']+$Margin, $Margin, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                                                                                                                                                       'BorderB' =>$BorderB));
         } elseif ($Style == LEGEND_BOX) {
             $this->drawFilledRectangle($Boundaries['L']-$Margin, $Boundaries['T']-$Margin, $Boundaries['R']+$Margin, $Boundaries['B']+$Margin, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                                                                                                                                       'BorderB' =>$BorderB));
         }
             ...
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 if (isset($Serie['Picture'])) {
                     ...
                     if ($Family == LEGEND_FAMILY_BOX) {
                         ...
                         $this->drawFilledRectangle($X+1+$XOffset, $Y+1+$YOffset, $X+$BoxWidth+$XOffset+1, $Y+$BoxHeight+1+$YOffset, array( 'R'     =>0, 'G' =>0, 'B' =>0,
                                                                                                                                            'Alpha' =>20));
                         $this->drawFilledRectangle($X+$XOffset, $Y+$YOffset, $X+$BoxWidth+$XOffset, $Y+$BoxHeight+$YOffset, array( 'R'           =>$R, 'G' =>$G, 'B' =>$B,
                                                                                                                                    'Surrounding' =>20));
                     } elseif ($Family == LEGEND_FAMILY_CIRCLE) {
                         $this->drawFilledCircle($X+1+$IconAreaWidth/2, $Y+1+$IconAreaHeight/2, min($IconAreaHeight/2, $IconAreaWidth/2), array( 'R'     =>0, 'G' =>0, 'B' =>0,
                                                                                                                                                 'Alpha' =>20));
                         $this->drawFilledCircle($X+$IconAreaWidth/2, $Y+$IconAreaHeight/2, min($IconAreaHeight/2, $IconAreaWidth/2), array( 'R'           =>$R, 'G' =>$G, 'B' =>$B,
                                                                                                                                             'Surrounding' =>20));
                     } elseif ($Family == LEGEND_FAMILY_LINE) {
                         $this->drawLine($X+1, $Y+1+$IconAreaHeight/2, $X+1+$IconAreaWidth, $Y+1+$IconAreaHeight/2, array( 'R'      =>0, 'G' =>0, 'B' =>0, 'Alpha' =>20, 'Ticks' =>$Ticks,
                                                                                                                           'Weight' =>$Weight));
                         $this->drawLine($X, $Y+$IconAreaHeight/2, $X+$IconAreaWidth, $Y+$IconAreaHeight/2, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Ticks' =>$Ticks,
                                                                                                                   'Weight' =>$Weight));
                     }
                 }
                 ...
                 if ($Mode == LEGEND_VERTICAL) {
                     $Lines = preg_split("/\n/", (string) $Serie['Description']);
                     foreach ($Lines as $Key => $Value) {
                         $this->drawText($X+$IconAreaWidth+4, $Y+$IconAreaHeight/2+(($this->FontSize+3)*$Key), $Value, array( 'R'        =>$FontR, 'G' =>$FontG, 'B' =>$FontB, 'Align' =>TEXT_ALIGN_MIDDLELEFT, 'FontSize' =>$FontSize,
                                                                                                                              'FontName' =>$FontName));
                     }
                     ...
                     $Width = [];
                     foreach ($Lines as $Key => $Value) {
                         $BoxArray = $this->drawText($X+$IconAreaWidth+4, $Y+$IconAreaHeight/2+(($this->FontSize+3)*$Key), $Value, array( 'R'        =>$FontR, 'G' =>$FontG, 'B' =>$FontB, 'Align' =>TEXT_ALIGN_MIDDLELEFT, 'FontSize' =>$FontSize,
                                                                                                                                          'FontName' =>$FontName));
                         $Width[] = $BoxArray[1]['X'];
                     }
                     $X=max($Width)+2+$XStep;
     ...
     public function drawScale($Format= '')
     {
         ...
         $ManualScale    = isset($Format['ManualScale']) ? $Format['ManualScale'] : array( '0' =>array( 'Min' =>-100,
                                                                                                        'Max' =>100));
         ...
         $SkippedAxisColor = array( 'R'     =>$SkippedAxisR, 'G' =>$SkippedAxisG, 'B' =>$SkippedAxisB, 'Alpha' =>$SkippedAxisAlpha,
         ...
         foreach ($Data['Axis'] as $AxisID => $Parameters) {
             if (isset($Parameters['Color'])) {
                 ...
                 $this->setFontProperties(array( 'R' => $Parameters['Color']['R'], 'G' => $Parameters['Color']['G'],
                 ...
                 if ($Pos == SCALE_POS_LEFTRIGHT) {
                     if ($Parameters['Position'] == AXIS_POSITION_BOTTOM) {
                         ...
                         if (!$RemoveXAxis) {
                             if ($Floating) {
                                 ...
                                 $this->drawLine($this->GraphAreaX1+ $Parameters['Margin'], $AxisPos['B'], $this->GraphAreaX2- $Parameters['Margin'], $AxisPos['B'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                                                                            'Alpha' =>$AxisAlpha));
                             } else {
                                 $FloatingOffset = 0;
                                 $this->drawLine($this->GraphAreaX1, $AxisPos['B'], $this->GraphAreaX2, $AxisPos['B'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                              'Alpha' =>$AxisAlpha));
                             }

                             if ($DrawArrows) {
                                 $this->drawArrow($this->GraphAreaX2- $Parameters['Margin'], $AxisPos['B'], $this->GraphAreaX2+($ArrowSize*2), $AxisPos['B'], array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                     'Size'  =>$ArrowSize));
                             }
                         }
                         ...
                         for ($i=0; $i<= $Parameters['Rows']; $i++) {
                             ...
                             if ($this->isValidLabel($Value, $LastValue, $LabelingMethod, $ID, $LabelSkip) && !$RemoveXAxis) {
                                 $Bounds    = $this->drawText($XPos, $YPos+$OuterTickWidth+$YLabelOffset, $Value, array( 'Angle' =>$LabelRotation,
                                                                                                                         'Align' =>$LabelAlign));
                             ...
                             if ($Skipped) {
                                 ...
                                 if ($DrawXLines && ($XPos != $this->GraphAreaX1 && $XPos != $this->GraphAreaX2)) {
                                     $this->drawLine($XPos, $this->GraphAreaY1+$FloatingOffset, $XPos, $this->GraphAreaY2-$FloatingOffset, array( 'R'     =>$GridR, 'G' =>$GridG, 'B' =>$GridB, 'Alpha' =>$GridAlpha,
                                                                                                                                                  'Ticks' =>$GridTicks));
                                 }
                                 if (($InnerTickWidth !=0 || $OuterTickWidth != 0) && !$RemoveXAxis) {
                                     $this->drawLine($XPos, $YPos-$InnerTickWidth, $XPos, $YPos+$OuterTickWidth, array( 'R'     =>$TickR, 'G' =>$TickG, 'B' =>$TickB,
                                                                                                                        'Alpha' =>$TickAlpha));
                                 }
                             }
                         }
                         ...
                         if (!$RemoveXAxis) {
                             if ($Floating) {
                                 $FloatingOffset = $YMargin;
                                 $this->drawLine($this->GraphAreaX1+ $Parameters['Margin'], $AxisPos['T'], $this->GraphAreaX2- $Parameters['Margin'], $AxisPos['T'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                                                                            'Alpha' =>$AxisAlpha));
                             } else {
                                 $FloatingOffset = 0;
                                 $this->drawLine($this->GraphAreaX1, $AxisPos['T'], $this->GraphAreaX2, $AxisPos['T'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                              'Alpha' =>$AxisAlpha));
                             }

                             if ($DrawArrows) {
                                 $this->drawArrow($this->GraphAreaX2- $Parameters['Margin'], $AxisPos['T'], $this->GraphAreaX2+($ArrowSize*2), $AxisPos['T'], array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                     'Size'  =>$ArrowSize));
                             }
                         }
                     ...
                     if ($Parameters['Position'] == AXIS_POSITION_LEFT) {
                         ...
                         if (!$RemoveXAxis) {
                             if ($Floating) {
                                 $FloatingOffset = $YMargin;
                                 $this->drawLine($AxisPos['L'], $this->GraphAreaY1+ $Parameters['Margin'], $AxisPos['L'], $this->GraphAreaY2- $Parameters['Margin'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                                                                            'Alpha' =>$AxisAlpha));
                             } else {
                                 $FloatingOffset = 0;
                                 $this->drawLine($AxisPos['L'], $this->GraphAreaY1, $AxisPos['L'], $this->GraphAreaY2, array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                              'Alpha' =>$AxisAlpha));
                             }

                             if ($DrawArrows) {
                                 $this->drawArrow($AxisPos['L'], $this->GraphAreaY2- $Parameters['Margin'], $AxisPos['L'], $this->GraphAreaY2+($ArrowSize*2), array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                     'Size'  =>$ArrowSize));
                             }
                         }
                         ...
                         for ($i=0; $i<= $Parameters['Rows']; $i++) {
                             ...
                             $Skipped = true;
                             if ($this->isValidLabel($Value, $LastValue, $LabelingMethod, $ID, $LabelSkip) && !$RemoveXAxis) {
                                 $Bounds  = $this->drawText($XPos-$OuterTickWidth+$XLabelOffset, $YPos, $Value, array( 'Angle' =>$LabelRotation,
                                                                                                                       'Align' =>$LabelAlign));
                             ...
                             if ($Skipped) {
                                 ...
                                 if ($DrawXLines && ($YPos != $this->GraphAreaY1 && $YPos != $this->GraphAreaY2)) {
                                     $this->drawLine($this->GraphAreaX1+$FloatingOffset, $YPos, $this->GraphAreaX2-$FloatingOffset, $YPos, array( 'R'     =>$GridR, 'G' =>$GridG, 'B' =>$GridB, 'Alpha' =>$GridAlpha,
                                                                                                                                                  'Ticks' =>$GridTicks));
                                 }
                                 if (($InnerTickWidth !=0 || $OuterTickWidth != 0) && !$RemoveXAxis) {
                                     $this->drawLine($XPos-$OuterTickWidth, $YPos, $XPos+$InnerTickWidth, $YPos, array( 'R'     =>$TickR, 'G' =>$TickG, 'B' =>$TickB,
                                                                                                                        'Alpha' =>$TickAlpha));
                                 }
                             }
                         }
                         ...
                         if (!$RemoveXAxis) {
                             if ($Floating) {
                                 $FloatingOffset = $YMargin;
                                 $this->drawLine($AxisPos['R'], $this->GraphAreaY1+ $Parameters['Margin'], $AxisPos['R'], $this->GraphAreaY2- $Parameters['Margin'], array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                                                                            'Alpha' =>$AxisAlpha));
                             } else {
                                 $FloatingOffset = 0;
                                 $this->drawLine($AxisPos['R'], $this->GraphAreaY1, $AxisPos['R'], $this->GraphAreaY2, array( 'R'     =>$AxisR, 'G' =>$AxisG, 'B' =>$AxisB,
                                                                                                                              'Alpha' =>$AxisAlpha));
                             }

                             if ($DrawArrows) {
                                 $this->drawArrow($AxisPos['R'], $this->GraphAreaY2- $Parameters['Margin'], $AxisPos['R'], $this->GraphAreaY2+($ArrowSize*2), array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                     'Size'  =>$ArrowSize));
                             }
                         }
             ...
             if ($Parameters['Identity'] == AXIS_Y) {
                 if ($Pos == SCALE_POS_LEFTRIGHT) {
                     if ($Parameters['Position'] == AXIS_POSITION_LEFT) {
                         ...
                         if ($DrawArrows) {
                             $this->drawArrow($AxisPos['L'], $this->GraphAreaY1+ $Parameters['Margin'], $AxisPos['L'], $this->GraphAreaY1-($ArrowSize*2), array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                 'Size'  =>$ArrowSize));
                         }
                         ...
                         for ($i=0; $i<= $Parameters['Rows']; $i++) {
                             ...
                             if ($i%2 == 1) {
                                 $BGColor = array( 'R'     =>$BackgroundR1, 'G' =>$BackgroundG1, 'B' =>$BackgroundB1,
                                                   'Alpha' =>$BackgroundAlpha1);
                             } else {
                                 $BGColor = array( 'R'     =>$BackgroundR2, 'G' =>$BackgroundG2, 'B' =>$BackgroundB2,
                                                   'Alpha' =>$BackgroundAlpha2);
                             }
                             if ($LastY != null && $CycleBackground && ($DrawYLines == ALL || in_array(
                                         $AxisID,
                                         $DrawYLines,
                                         true
                                     ))) {
                             ...
                             if ($DrawYLines == ALL || in_array($AxisID, $DrawYLines, true)) {
                                 $this->drawLine($this->GraphAreaX1+$FloatingOffset, $YPos, $this->GraphAreaX2-$FloatingOffset, $YPos, array( 'R'     =>$GridR, 'G' =>$GridG, 'B' =>$GridB, 'Alpha' =>$GridAlpha,
                                                                                                                                              'Ticks' =>$GridTicks));
                             }

                             if ($DrawSubTicks && $i != $Parameters['Rows']) {
                                 $this->drawLine($XPos-$OuterSubTickWidth, $YPos-$SubTicksSize, $XPos+$InnerSubTickWidth, $YPos-$SubTicksSize, array( 'R'     =>$SubTickR, 'G' =>$SubTickG, 'B' =>$SubTickB,
                                                                                                                                                      'Alpha' =>$SubTickAlpha));
                             }
                         ...
                         if ($DrawArrows) {
                             $this->drawArrow($AxisPos['R'], $this->GraphAreaY1+ $Parameters['Margin'], $AxisPos['R'], $this->GraphAreaY1-($ArrowSize*2), array( 'FillR' =>$AxisR, 'FillG' =>$AxisG, 'FillB' =>$AxisB,
                                                                                                                                                                 'Size'  =>$ArrowSize));
                         }
                         ...
                         for ($i=0; $i<= $Parameters['Rows']; $i++) {
                             ...
                             $this->drawLine($XPos-$InnerTickWidth, $YPos, $XPos+$OuterTickWidth, $YPos, array( 'R'     =>$TickR, 'G' =>$TickG, 'B' =>$TickB,
                                                                                                                'Alpha' =>$TickAlpha));
                     ...
                     if ($Parameters['Position'] == AXIS_POSITION_TOP) {
                         ...
                         for ($i=0; $i<= $Parameters['Rows']; $i++) {
                             ...
                             if ($LastX != null && $CycleBackground && ($DrawYLines == ALL || in_array(
                                         $AxisID,
                                         $DrawYLines,
                                         true
                                     ))) {
                             ...
                             if ($DrawSubTicks && $i != $Parameters['Rows']) {
                                 $this->drawLine($XPos+$SubTicksSize, $YPos-$OuterSubTickWidth, $XPos+$SubTicksSize, $YPos+$InnerSubTickWidth, array( 'R'     =>$SubTickR, 'G' =>$SubTickG, 'B' =>$SubTickB,
                                                                                                                                                      'Alpha' =>$SubTickAlpha));
                             }

                             $this->drawLine($XPos, $YPos-$OuterTickWidth, $XPos, $YPos+$InnerTickWidth, array( 'R'     =>$TickR, 'G' =>$TickG, 'B' =>$TickB,
                                                                                                                'Alpha' =>$TickAlpha));
     ...
     public function drawXThreshold($Value, $Format= '')
     {
         ...
         $CaptionSettings = array( 'DrawBox'    =>$DrawBox, 'DrawBoxBorder' =>$DrawBoxBorder, 'BorderOffset' =>$BorderOffset, 'BoxRounded' =>$BoxRounded, 'RoundedRadius' =>$RoundedRadius,
                                   ...
                                   'R'          =>$CaptionR, 'G' =>$CaptionG, 'B' =>$CaptionB,
                                   'Alpha'      =>$CaptionAlpha);
         ...
         if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
             ...
             if ($XPos >= $this->GraphAreaX1 + $AbscissaMargin && $XPos <= $this->GraphAreaX2 - $AbscissaMargin) {
                 $this->drawLine($XPos, $YPos1, $XPos, $YPos2, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                                      'Weight' =>$Weight));

                 if ($Wide) {
                     $this->drawLine($XPos-1, $YPos1, $XPos-1, $YPos2, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                              'Ticks' =>$Ticks));
                     $this->drawLine($XPos+1, $YPos1, $XPos+1, $YPos2, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                              'Ticks' =>$Ticks));
                 }
             ...
             if ($XPos >= $this->GraphAreaY1 + $AbscissaMargin && $XPos <= $this->GraphAreaY2 - $AbscissaMargin) {
                 $this->drawLine($YPos1, $XPos, $YPos2, $XPos, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                                      'Weight' =>$Weight));

                 if ($Wide) {
                     $this->drawLine($YPos1, $XPos-1, $YPos2, $XPos-1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                              'Ticks' =>$Ticks));
                     $this->drawLine($YPos1, $XPos+1, $YPos2, $XPos+1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                              'Ticks' =>$Ticks));
                 }
     ...
     public function drawXThresholdArea($Value1, $Value2, $Format= '')
     {
         ...
         if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
             ...
             if ($Border) {
                 $this->drawLine($XPos1, $YPos1, $XPos1, $YPos2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
                 $this->drawLine($XPos2, $YPos1, $XPos2, $YPos2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
             }
             ...
             if ($AreaName != null) {
                 ...
                 $this->Shadow = $RestoreShadow;
                 $this->drawText($XPos, $YPos, $AreaName, array( 'R'     =>$NameR, 'G' =>$NameG, 'B' =>$NameB, 'Alpha' =>$NameAlpha, 'Angle' =>$NameAngle,
             ...
             if ($Border) {
                 $this->drawLine($YPos1, $XPos1, $YPos2, $XPos1, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
                 $this->drawLine($YPos1, $XPos2, $YPos2, $XPos2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
             }
             ...
             if ($AreaName != null) {
                 ...
                 $this->Shadow = $RestoreShadow;
                 $this->drawText($YPos, $XPos, $AreaName, array( 'R'     =>$NameR, 'G' =>$NameG, 'B' =>$NameB, 'Alpha' =>$NameAlpha, 'Angle' =>0,
                                                                 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE));
                 if ($DisableShadowOnArea) {
                     $this->Shadow = false;
                 }
     ...
     public function drawThreshold($Value, $Format= '')
     {
         ...
         if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
             ...
             if ($YPos >= $this->GraphAreaY1+ $Data['Axis'][$AxisID]['Margin'] && $YPos <= $this->GraphAreaY2- $Data['Axis'][$AxisID]['Margin']) {
                 ...
                 $this->drawLine($X1, $YPos, $X2, $YPos, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                                'Weight' =>$Weight));

                 if ($Wide) {
                     $this->drawLine($X1, $YPos-1, $X2, $YPos-1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                        'Ticks' =>$Ticks));
                     $this->drawLine($X1, $YPos+1, $X2, $YPos+1, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                        'Ticks' =>$Ticks));
                 }
         ...
         if ($Data['Orientation'] == SCALE_POS_TOPBOTTOM) {
             ...
             if ($XPos >= $this->GraphAreaX1+ $Data['Axis'][$AxisID]['Margin'] && $XPos <= $this->GraphAreaX2- $Data['Axis'][$AxisID]['Margin']) {
                 ...
                 $this->drawLine($XPos, $Y1, $XPos, $Y2, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                                'Weight' =>$Weight));

                 if ($Wide) {
                     $this->drawLine($XPos-1, $Y1, $XPos-1, $Y2, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                        'Ticks' =>$Ticks));
                     $this->drawLine($XPos+1, $Y1, $XPos+1, $Y2, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/$WideFactor,
                                                                        'Ticks' =>$Ticks));
                 }
     ...
     public function drawThresholdArea($Value1, $Value2, $Format= '')
     {
         ...
         if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
             ...
             $this->drawFilledRectangle($XPos1, $YPos1, $XPos2, $YPos2, array( 'R' =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha));
             if ($Border) {
                 $this->drawLine($XPos1, $YPos1, $XPos2, $YPos1, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
                 $this->drawLine($XPos1, $YPos2, $XPos2, $YPos2, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                        'Ticks' =>$BorderTicks));
             }
             ...
             if ($AreaName != null) {
                 ...
                 $YPos = ($YPos2-$YPos1)/2 + $YPos1;
                 $this->Shadow = $RestoreShadow;
                 $this->drawText($XPos, $YPos, $AreaName, array( 'R'     =>$NameR, 'G' =>$NameG, 'B' =>$NameB, 'Alpha' =>$NameAlpha, 'Angle' =>0,
                                                                 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE));
                 if ($DisableShadowOnArea) {
                     $this->Shadow = false;
                 }
             ...
             if ($AreaName != null) {
                 ...
                 $this->Shadow = $RestoreShadow;
                 $this->drawText($YPos, $XPos, $AreaName, array( 'R'     =>$NameR, 'G' =>$NameG, 'B' =>$NameB, 'Alpha' =>$NameAlpha, 'Angle' =>$NameAngle,
                                                                 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE));
                 if ($DisableShadowOnArea) {
                     $this->Shadow = false;
                 }
     ...
     public function writeBounds($Type=BOUND_BOTH, $Format=null)
     {
         ...
         $CaptionSettings = array( 'DrawBox'        =>$DrawBox, 'DrawBoxBorder' =>$DrawBoxBorder, 'BorderOffset' =>$BorderOffset, 'BoxRounded' =>$BoxRounded, 'RoundedRadius' =>$RoundedRadius,
                                   'BoxR'           =>$BoxR, 'BoxG' =>$BoxG, 'BoxB' =>$BoxB, 'BoxAlpha' =>$BoxAlpha, 'BoxSurrounding' =>$BoxSurrounding,
                                   'BoxBorderR'     =>$BoxBorderR, 'BoxBorderG' =>$BoxBorderG, 'BoxBorderB' =>$BoxBorderB,
             ...
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     ...
                     foreach ($PosArray as $Key => $Y) {
                         if ($DisplayValues) {
                             $this->drawText($X, $Y-$DisplayOffset-$SerieWeight-$BorderSize-$PicOffset, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'     =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                                                                'Align' =>TEXT_ALIGN_BOTTOMMIDDLE));
                         }
                     ...
                     foreach ($PosArray as $Key => $X) {
                         if ($DisplayValues) {
                             $this->drawText($X+$DisplayOffset+$SerieWeight+$BorderSize+$PicOffset, $Y, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'Angle' =>270, 'R' =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                                                                'Align' =>TEXT_ALIGN_BOTTOMMIDDLE));
                         }
         ...
         foreach ($Data['Series'] as $SerieName => $Serie) {
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 if ($BreakR == null) {
                     ...
                     $BreakSettings = array( 'R'      =>$BreakR, 'G' =>$BreakG, 'B' =>$BreakB, 'Alpha' =>$Alpha, 'Ticks' =>$VoidTicks,
                                             'Weight' =>$Weight);
                 }
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     ...
                     foreach ($PosArray as $Key => $Y) {
                         if ($DisplayValues) {
                             $this->drawText($X, $Y-$DisplayOffset, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'     =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                            'Align' =>TEXT_ALIGN_BOTTOMMIDDLE));
                         }
                     ...
                     foreach ($PosArray as $Key => $X) {
                         if ($DisplayValues) {
                             $this->drawText($X+$DisplayOffset, $Y, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'Angle' =>270, 'R' =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                            'Align' =>TEXT_ALIGN_BOTTOMMIDDLE));
                         }
                         ...
                         if ($Y == VOID) {
                             ...
                             if ($Area != '') {
                                 foreach ($Area as $key => $Points) {
                                     ...
                                     $this->drawPolygonChart($Corners, array( 'R'         =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha/2, 'NoBorder' =>true,
                                                                              'Threshold' =>$Threshold));
                                 }
                                 $this->drawSpline($WayPoints, array( 'Force' =>$Force, 'R' =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha,
                                                                      'Ticks' =>$Ticks));
                             }
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     ...
                     foreach ($PosArray as $Key => $Y) {
                         if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                             ...
                             $this->drawText($X, $Y-$Offset-$Weight, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'     =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                             'Align' =>$Align));
                         }
                         ...
                         if ($Y != VOID && $LastX != null && $LastY != null) {
                             $this->drawLine($LastX, $LastY, $X, $Y, array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                                            'Weight' =>$Weight));
                         }
                     ...
                     foreach ($PosArray as $Key => $X) {
                         if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                             $this->drawText($X+$DisplayOffset+$Weight, $Y, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'Angle' =>270, 'R' =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                                    'Align' =>TEXT_ALIGN_BOTTOMMIDDLE));
                         }
     ...
     public function drawZoneChart($SerieA, $SerieB, $Format=null)
     {
         ...
         if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
             ...
             for ($i=0;$i<=$BoundsACount-4;$i=$i+2) {
                 $this->drawLine($BoundsA[$i], $BoundsA[$i+1], $BoundsA[$i+2], $BoundsA[$i+3], array( 'R'     =>$LineR, 'G' =>$LineG, 'B' =>$LineB, 'Alpha' =>$LineAlpha,
                                                                                                      'Ticks' =>$LineTicks));
                 $this->drawLine($BoundsB[$i], $BoundsB[$i+1], $BoundsB[$i+2], $BoundsB[$i+3], array( 'R'     =>$LineR, 'G' =>$LineG, 'B' =>$LineB, 'Alpha' =>$LineAlpha,
                                                                                                      'Ticks' =>$LineTicks));
             }
         } else {
             if ($XDivs == 0) {
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     ...
                     foreach ($PosArray as $Key => $X) {
                         if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                             ...
                             $this->drawText($X+$Offset+$Weight, $Y, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'     =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                             'Align' =>$Align));
                         }
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     ...
                     foreach ($PosArray as $Key => $Y) {
                         if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                             ...
                             $this->drawText($X, $Y-$Offset, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'     =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                     'Align' =>$Align));
                         }
                     ...
                     if ($this->Shadow) {
                         ...
                         foreach ($ShadowArea as $Key => $Points) {
                             $this->drawPolygonChart($Points, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB,
                                                                     'Alpha' =>$this->Shadowa));
                         }
                     }
                     ...
                     foreach ($PosArray as $Key => $X) {
                         if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                             ...
                             $this->drawText($X+$Offset, $Y, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'Angle' =>270, 'R' =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB,
                                                                                                                                     'Align' =>$Align));
                         }
     ...
     public function drawBarChart($Format=null)
     {
         ...
         foreach ($Data['Series'] as $SerieName => $Serie) {
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 $Color = array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                 'BorderB' =>$BorderB);
                     ...
                     foreach ($PosArray as $Key => $Y2) {
                         ...
                         if ($OverrideColors != null) {
                             if (isset($OverrideColors[$ID])) {
                                 $Color = array( 'R'       => $OverrideColors[$ID]['R'], 'G' => $OverrideColors[$ID]['G'], 'B' => $OverrideColors[$ID]['B'], 'Alpha' => $OverrideColors[$ID]['Alpha'], 'BorderR' => $OverrideColors[$ID]['BorderR'], 'BorderG' => $OverrideColors[$ID]['BorderG'],
                         ...
                         if ($Y2 != VOID) {
                                 ...
                                 if ($Rounded) {
                                     ...
                                     if ($Gradient) {
                                         ...
                                         if ($GradientMode == GRADIENT_SIMPLE) {
                                             if ($Serie['Data'][$Key] >= 0) {
                                                 $GradienColor = array( 'StartR' =>$GradientStartR, 'StartG' =>$GradientStartG, 'StartB' =>$GradientStartB, 'EndR' =>$GradientEndR, 'EndG' =>$GradientEndG, 'EndB' =>$GradientEndB,
                                                                        'Alpha'  =>$GradientAlpha);
                                             } else {
                                                 $GradienColor = array( 'StartR' =>$GradientEndR, 'StartG' =>$GradientEndG, 'StartB' =>$GradientEndB, 'EndR' =>$GradientStartR, 'EndG' =>$GradientStartG, 'EndB' =>$GradientStartB,
                                                                        'Alpha'  =>$GradientAlpha);
                                             }
                                             ...
                                             $GradienColor1 = array( 'StartR' =>$GradientEndR, 'StartG' =>$GradientEndG, 'StartB' =>$GradientEndB, 'EndR' =>$GradientStartR, 'EndG' =>$GradientStartG, 'EndB' =>$GradientStartB,
                                                                     'Alpha'  =>$GradientAlpha);
                                             $GradienColor2 = array( 'StartR' =>$GradientStartR, 'StartG' =>$GradientStartG, 'StartB' =>$GradientStartB, 'EndR' =>$GradientEndR, 'EndG' =>$GradientEndG, 'EndB' =>$GradientEndB,
                                                                     'Alpha'  =>$GradientAlpha);
                                             $XSpan = floor($XSize / 3);
                             ...
                             if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                                 ...
                                 if ($DisplayPos == LABEL_POS_INSIDE && abs($TxtHeight) < abs($BarHeight)) {
                                     ...
                                     $this->drawText($X+$XOffset+$XSize/2, $Y2-$Offset, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'        =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB, 'Align' =>$Align,
                         ...
                         if ($X2 != VOID) {
                             ...
                             if ($DisplayValues && $Serie['Data'][$Key] != VOID) {
                                 ...
                                 if ($DisplayPos == LABEL_POS_INSIDE && abs($TxtWidth) < abs($BarWidth)) {
                                     ...
                                     $this->drawText($CenterX, $CenterY, $Caption, array( 'R'        =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB, 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE,
                                                                                          'FontSize' =>$DisplaySize));
                                 } else {
                                     if ($Serie['Data'][$Key] >= 0) {
                                         $Align = TEXT_ALIGN_MIDDLELEFT;
                                     ...
                                     $this->drawText($X2+$Offset, $Y+$YOffset+$YSize/2, $Caption, array( 'R'        =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB, 'Align' =>$Align,
                                                                                                         'FontSize' =>$DisplaySize));
                                 }
     ...
     public function drawStackedBarChart($Format=null)
     {
         ...
         foreach ($Data['Series'] as $SerieName => $Serie) {
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 $Color = array( 'TransCorner' =>true, 'R' =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'BorderR' =>$BorderR, 'BorderG' =>$BorderG,
                                 'BorderB'     =>$BorderB);
                         ...
                         if ($Height != VOID && $Serie['Data'][$Key] != 0) {
                             ...
                             if ($Rounded) {
                                 ...
                                 if ($Gradient) {
                                     ...
                                     if ($GradientMode == GRADIENT_SIMPLE) {
                                         $GradientColor = array( 'StartR' =>$GradientStartR, 'StartG' =>$GradientStartG, 'StartB' =>$GradientStartB, 'EndR' =>$GradientEndR, 'EndG' =>$GradientEndG, 'EndB' =>$GradientEndB,
                                                                 'Alpha'  =>$GradientAlpha);
                                         $this->drawGradientArea($X+$XOffset, $Y1-1-$YSpaceUp+$YSpaceDown, $X+$XOffset+$XSize, $Y2+1, DIRECTION_VERTICAL, $GradientColor);
                                     } elseif ($GradientMode == GRADIENT_EFFECT_CAN) {
                                         $GradientColor1 = array( 'StartR' =>$GradientEndR, 'StartG' =>$GradientEndG, 'StartB' =>$GradientEndB, 'EndR' =>$GradientStartR, 'EndG' =>$GradientStartG, 'EndB' =>$GradientStartB,
                                                                  'Alpha'  =>$GradientAlpha);
                                         $GradientColor2 = array( 'StartR' =>$GradientStartR, 'StartG' =>$GradientStartG, 'StartB' =>$GradientStartB, 'EndR' =>$GradientEndR, 'EndG' =>$GradientEndG, 'EndB' =>$GradientEndB,
                                                                  'Alpha'  =>$GradientAlpha);
                                         $XSpan = floor($XSize / 3);
                             ...
                             if ($DisplayValues) {
                                 ...
                                 if ($DisplayOrientation == ORIENTATION_HORIZONTAL || $DisplayOrientation == ORIENTATION_AUTO) {
                                     if ($TxtHeight < $BarHeight && $TxtWidth < $BarWidth) {
                                         $this->drawText($XCenter, $YCenter, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'        =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB, 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE, 'FontSize' =>$DisplaySize,
                                 ...
                                 if ($DisplayOrientation == ORIENTATION_VERTICAL || ($DisplayOrientation == ORIENTATION_AUTO && !$Done)) {
                                     if ($TxtHeight < $BarWidth && $TxtWidth < $BarHeight) {
                                         $this->drawText($XCenter, $YCenter, $this->scaleFormat($Serie['Data'][$Key], $Mode, $Format, $Unit), array( 'R'        =>$DisplayR, 'G' =>$DisplayG, 'B' =>$DisplayB, 'Angle' =>90, 'Align' =>TEXT_ALIGN_MIDDLEMIDDLE, 'FontSize' =>$DisplaySize,
                                                                                                                                                     'FontName' =>$DisplayFont));
                                     }
                                 }
                             }
         ...
         foreach ($SerieOrder as $Key => $SerieName) {
             ...
             if ($Serie['isDrawable'] == true && $SerieName != $Data['Abscissa']) {
                 ...
                 if ($LineSurrounding != null) {
                     $LineColor = array( 'R'     =>$R+$LineSurrounding, 'G' =>$G+$LineSurrounding, 'B' =>$B+$LineSurrounding,
                                         'Alpha' =>$Alpha);
                 } elseif ($LineR != VOID) {
                 ...
                 if ($PlotBorderSurrounding != null) {
                     $PlotBorderColor = array( 'R'     =>$R+$PlotBorderSurrounding, 'G' =>$G+$PlotBorderSurrounding, 'B' =>$B+$PlotBorderSurrounding,
                 ...
                 if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                     if ($Caption) {
                         if ($CaptionLine) {
                             ...
                             $CaptionSettings = array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                       'Weight' =>$Weight);
                             if ($CaptionBox) {
                                 $this->drawFilledRectangle($StartX, $YPos, $EndX, $YPos+$CaptionHeight, array( 'R'     =>$CaptionFillR, 'G' =>$CaptionFillG, 'B' =>$CaptionFillB, 'BorderR' =>$CaptionBorderR, 'BorderG' =>$CaptionBorderG, 'BorderB' =>$CaptionBorderB,
                                                                                                                'Alpha' =>$CaptionFillAlpha));
                             }
                             $this->drawLine($StartX+2, $YPos+($CaptionHeight/2), $EndX-2, $YPos+($CaptionHeight/2), $CaptionSettings);
                         } else {
                             $this->drawFilledRectangle($this->GraphAreaX1-$CaptionWidth+$XMargin-$CaptionMargin, $YPos, $this->GraphAreaX1-$CaptionMargin+$XMargin, $YPos+$CaptionHeight, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'BorderR' =>$CaptionBorderR, 'BorderG' =>$CaptionBorderG,
                     ...
                     if ($DrawBackground) {
                         $this->drawFilledRectangle($StartX-1, $TopY-1, $EndX+1, $BottomY+1, array( 'R'     =>$BackgroundR, 'G' =>$BackgroundG, 'B' =>$BackgroundB,
                                                                                                    'Alpha' =>$BackgroundAlpha));
                     }
                     if ($DrawBorder) {
                         $this->drawRectangle($StartX-1, $TopY-1, $EndX+1, $BottomY+1, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                              'Alpha' =>$BorderAlpha));
                     }
                     ...
                     foreach ($PosArray as $Key => $Y) {
                         if ($Y != VOID && $LastY != null) {
                             ...
                             if ($ShadedSlopeBox && $LastColor != null) { // && $Slope != 0
                                 $GradientSettings = array( 'StartR' => $LastColor['R'], 'StartG' => $LastColor['G'], 'StartB' => $LastColor['B'], 'EndR' =>$R, 'EndG' =>$G,
                                                            'EndB'   =>$B);
                     ...
                     if ($Caption) {
                         ...
                         if ($CaptionLine) {
                             $CaptionSettings = array( 'R'      =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha, 'Ticks' =>$Ticks,
                                                       'Weight' =>$Weight);
                             if ($CaptionBox) {
                                 $this->drawFilledRectangle($XPos, $StartY, $XPos+$CaptionHeight, $EndY, array( 'R'     =>$CaptionFillR, 'G' =>$CaptionFillG, 'B' =>$CaptionFillB, 'BorderR' =>$CaptionBorderR, 'BorderG' =>$CaptionBorderG, 'BorderB' =>$CaptionBorderB,
                                                                                                                'Alpha' =>$CaptionFillAlpha));
                             }
                             $this->drawLine($XPos+($CaptionHeight/2), $StartY+2, $XPos+($CaptionHeight/2), $EndY-2, $CaptionSettings);
                         } else {
                             $this->drawFilledRectangle($XPos, $StartY, $XPos+$CaptionHeight, $EndY, array( 'R'       =>$R, 'G' =>$G, 'B' =>$B, 'BorderR' =>$CaptionBorderR, 'BorderG' =>$CaptionBorderG,
                                                                                                            'BorderB' =>$CaptionBorderB));
                         }
                     }
                     ...
                     if ($DrawBackground) {
                         $this->drawFilledRectangle($TopX-1, $StartY-1, $BottomX+1, $EndY+1, array( 'R'     =>$BackgroundR, 'G' =>$BackgroundG, 'B' =>$BackgroundB,
                                                                                                    'Alpha' =>$BackgroundAlpha));
                     }
                     if ($DrawBorder) {
                         $this->drawRectangle($TopX-1, $StartY-1, $BottomX+1, $EndY+1, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                              'Alpha' =>$BorderAlpha));
                     }
     ...
     public function writeLabel($SeriesName, $Indexes, $Format= '')
     {
         ...
         foreach ($Indexes as $Key => $Index) {
             ...
             if ($Data['Orientation'] == SCALE_POS_LEFTRIGHT) {
                 ...
                 if ($DrawVerticalLine) {
                     $this->drawLine($X, $this->GraphAreaY1+ $Data['YMargin'], $X, $this->GraphAreaY2- $Data['YMargin'], array( 'R'     =>$VerticalLineR, 'G' =>$VerticalLineG, 'B' =>$VerticalLineB, 'Alpha' =>$VerticalLineAlpha,
                 ...
                 foreach ($SeriesName as $iKey => $SerieName) {
                     if (isset($Data['Series'][$SerieName]['Data'][$Index])) {
                         ...
                         if ($DrawPoint == LABEL_POINT_CIRCLE) {
                             $this->drawFilledCircle($X, $Y, 3, array( 'R'       =>255, 'G' =>255, 'B' =>255, 'BorderR' =>0, 'BorderG' =>0,
                                                                       'BorderB' =>0));
                         } elseif ($DrawPoint == LABEL_POINT_BOX) {
                             $this->drawFilledRectangle($X-2, $Y-2, $X+2, $Y+2, array( 'R'       =>255, 'G' =>255, 'B' =>255, 'BorderR' =>0, 'BorderG' =>0,
                                                                                       'BorderB' =>0));
                         }
                 ...
                 if ($DrawVerticalLine) {
                     $this->drawLine($this->GraphAreaX1+ $Data['YMargin'], $Y, $this->GraphAreaX2- $Data['YMargin'], $Y, array( 'R'     =>$VerticalLineR, 'G' =>$VerticalLineG, 'B' =>$VerticalLineB, 'Alpha' =>$VerticalLineAlpha,
                                                                                                                                'Ticks' =>$VerticalLineTicks));
                 }
     ...
     public function drawLabelBox($X, $Y, $Title, $Captions, $Format= '')
     {
         ...
         if ($this->Shadow == true) {
             ...
             $Poly[] = $X-5+$this->ShadowX;
             $Poly[] = $Y-5+$this->ShadowX;
             $this->drawPolygon($Poly, array( 'R'     =>$this->ShadowR, 'G' =>$this->ShadowG, 'B' =>$this->ShadowB,
                                              'Alpha' =>$this->Shadowa));
         }
         ...
         $Poly[] = $X+5;
         $Poly[] = $Y-5;
         $this->drawPolygon($Poly, array( 'R'        =>$GradientEndR, 'G' =>$GradientEndG, 'B' =>$GradientEndB, 'Alpha' =>$BoxAlpha,
                                          'NoBorder' =>true));
         ...
         if (!$NoTitle) {
             $this->drawText($XMin+$VerticalMargin, $Y-7-$CaptionHeight-$HorizontalMargin*2, $Title, array( 'Align' =>TEXT_ALIGN_BOTTOMLEFT, 'R' =>$TitleR, 'G' =>$TitleG,
                                                                                                            'B'     =>$TitleB));
         }
         ...
         foreach ($Captions as $Key => $Caption) {
             ...
             if ($DrawSerieColor) {
                 $BoxSettings = array( 'R'       => $Caption['Format']['R'], 'G' => $Caption['Format']['G'], 'B' => $Caption['Format']['B'], 'Alpha' => $Caption['Format']['Alpha'], 'BorderR' =>0, 'BorderG' =>0,
                                       'BorderB' =>0);
     ...
     public function drawShape($X, $Y, $Shape, $PlotSize, $PlotBorder, $BorderSize, $R, $G, $B, $Alpha, $BorderR, $BorderG, $BorderB, $BorderAlpha)
     {
         if ($Shape == SERIE_SHAPE_FILLEDCIRCLE) {
             if ($PlotBorder) {
                 $this->drawFilledCircle($X, $Y, $PlotSize+$BorderSize, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                               'Alpha' =>$BorderAlpha));
             }
             $this->drawFilledCircle($X, $Y, $PlotSize, array( 'R' =>$R, 'G' =>$G, 'B' =>$B, 'Alpha' =>$Alpha));
         } elseif ($Shape == SERIE_SHAPE_FILLEDSQUARE) {
             if ($PlotBorder) {
                 $this->drawFilledRectangle($X-$PlotSize-$BorderSize, $Y-$PlotSize-$BorderSize, $X+$PlotSize+$BorderSize, $Y+$PlotSize+$BorderSize, array( 'R'     =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB,
                                                                                                                                                           'Alpha' =>$BorderAlpha));
             }
             $this->drawFilledRectangle($X-$PlotSize, $Y-$PlotSize, $X+$PlotSize, $Y+$PlotSize, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B,
                                                                                                       'Alpha' =>$Alpha));
             ...
             $this->drawLine($X, $Y-$PlotSize, $X-$PlotSize, $Y+$PlotSize, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B,
                                                                                  'Alpha' =>$Alpha));
             $this->drawLine($X-$PlotSize, $Y+$PlotSize, $X+$PlotSize, $Y+$PlotSize, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B,
                                                                                            'Alpha' =>$Alpha));
             $this->drawLine($X+$PlotSize, $Y+$PlotSize, $X, $Y-$PlotSize, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B,
                                                                                  'Alpha' =>$Alpha));
         } elseif ($Shape == SERIE_SHAPE_SQUARE) {
             $this->drawRectangle($X-$PlotSize, $Y-$PlotSize, $X+$PlotSize, $Y+$PlotSize, array( 'R'     =>$R, 'G' =>$G, 'B' =>$B,
                                                                                                 'Alpha' =>$Alpha));
             ...
             $Pos[]=$X;
             $Pos[]=$Y+$PlotSize;
             $this->drawPolygon($Pos, array( 'NoFill'      =>true, 'BorderR' =>$R, 'BorderG' =>$G, 'BorderB' =>$B,
                                             'BorderAlpha' =>$Alpha));
     ...
     public function drawPolygonChart($Points, $Format= '')
     {
                 ...
                 foreach ($Segments as $Key => $Coords) {
                     ...
                     if (min($Y1, $Y2) <= $Y && max($Y1, $Y2) >= $Y) {
                         ...
                         if (!is_array($Intersections)) {
                             $Intersections[] = $X;
                         } elseif (!in_array($X, $Intersections, true)) {
                             $Intersections[] = $X;
                         } elseif (in_array($X, $Intersections, true)) {
                             if ($Y == $DebugLine) {
                         ...
                         if (is_array($Intersections) && in_array(
                                 $X,
                                 $Intersections,
                                 true
                             ) && $LastSlope === '=' && ($Slope === '-')) {
         ...
         if (!$NoBorder) {
             foreach ($Segments as $Key => $Coords) {
                 $this->drawLine($Coords['X1'], $Coords['Y1'], $Coords['X2'], $Coords['Y2'], array( 'R'         =>$BorderR, 'G' =>$BorderG, 'B' =>$BorderB, 'Alpha' =>$BorderAlpha,
                                                                                                    'Threshold' =>$Threshold));
             }
         }
