 ...
 class pSpring
 {
                 ...
                 foreach ($this->Data as $Key2 => $Settings2) {
                     if ($Key != $Key2) {
                         ...
                         if ($Distance < $FreeZone) {
                             ...
                             if ($Force > 1) {
                                 $this->Data[$Key]['Vectors'][] = array( 'Type'  => 'R', 'Angle' =>$Angle % 360,
                                                                         'Force' =>$Force);
                             }
                         }
                     }
                 ...
                 if (isset($Settings['Connections'])) {
                     foreach ($Settings['Connections'] as $ID => $NodeID) {
                         if (isset($this->Data[$NodeID])) {
                             ...
                             if ($Force > 1) {
                                 $this->Data[$Key]['Vectors'][] = array( 'Type'  => 'A', 'Angle' =>$Angle % 360,
                                                                         'Force' =>$Force);
                             }
                         }
                     }
             ...
             if (isset($Settings['Connections'])) {
                 foreach ($Settings['Connections'] as $ID => $NodeID) {
                     if (isset($this->Data[$NodeID])) {
                         ...
                         $Links[] = array( 'X1'          =>$X1, 'Y1' =>$Y1, 'X2' =>$X2, 'Y2' =>$Y2, 'Source' => $Settings['Name'],
     ...
     public function drawSpring($Object, $Settings= '')
     {
             ...
             if (isset($Settings['Connections'])) {
                 foreach ($Settings['Connections'] as $ID => $NodeID) {
                     ...
                     if (isset($this->Data[$NodeID]) && !isset($Drawn[$Key][$NodeID]) && !isset($Drawn[$NodeID][$Key])) {
                         $Color = array( 'R'     => $this->Default['LinkR'], 'G' => $this->Default['LinkG'], 'B' => $this->Default['LinkB'],
                                         'Alpha' => $this->Default['Alpha']);

                         if ($this->Links != '') {
                             if (isset($this->Links[$Key][$NodeID]['R'])) {
                                 $Color = array( 'R'     => $this->Links[$Key][$NodeID]['R'], 'G' => $this->Links[$Key][$NodeID]['G'], 'B' => $this->Links[$Key][$NodeID]['B'],
         ...
         foreach ($this->Data as $Key => $Settings) {
             ...
             $Color     = array( 'R'          => $Settings['R'], 'G' => $Settings['G'], 'B' => $Settings['B'], 'Alpha' => $Settings['Alpha'], 'BorderR' => $Settings['BorderR'], 'BorderG' => $Settings['BorderG'], 'BorderB' => $Settings['BorderB'],
             ...
             if ($Name != '') {
                 $LabelOptions = array( 'R'     => $this->Labels['R'], 'G' => $this->Labels['G'], 'B' => $this->Labels['B'],
             ...
             foreach ($this->Data as $Key => $Settings) {
                 ...
                 if (isset($Settings['Vectors']) && $Settings['Type'] != NODE_TYPE_CENTRAL) {
                     foreach ($Settings['Vectors'] as $ID => $Vector) {
                         ...
                         $Factor = $Type === 'A' ? $this->MagneticForceA : $this->MagneticForceR;
                         $Color  = $Type === 'A'
                             ? array( 'FillR' =>255, 'FillG' =>0, 'FillB' =>0) : array( 'FillR' =>0, 'FillG' =>255,
                                                                                        'FillB' =>0);
