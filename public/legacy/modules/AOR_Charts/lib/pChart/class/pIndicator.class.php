     ...
     foreach ($IndicatorSections as $Key => $Settings)
      {
       ...
       if ( $CaptionColorFactor == NULL )
        { $CaptionColor    = array( 'Align' =>TEXT_ALIGN_TOPLEFT, 'FontName' =>$CaptionFontName, 'FontSize' =>$CaptionFontSize, 'R' =>$CaptionR, 'G' =>$CaptionG, 'B' =>$CaptionB,
                                    'Alpha' =>$CaptionAlpha); }
       else
        { $CaptionColor    = array( 'Align' =>TEXT_ALIGN_TOPLEFT, 'FontName' =>$CaptionFontName, 'FontSize' =>$CaptionFontSize, 'R' => $Settings['R']+$CaptionColorFactor, 'G' => $Settings['G']+$CaptionColorFactor,
                                    'B'     => $Settings['B']+$CaptionColorFactor); }

       if ( $SubCaptionColorFactor == NULL )
        $SubCaptionColor = array( 'Align' =>TEXT_ALIGN_TOPLEFT, 'FontName' =>$CaptionFontName, 'FontSize' =>$CaptionFontSize, 'R' =>$SubCaptionR, 'G' =>$SubCaptionG, 'B' =>$SubCaptionB,
                                  'Alpha' =>$SubCaptionAlpha);
       else
        $SubCaptionColor = array( 'Align' =>TEXT_ALIGN_TOPLEFT, 'FontName' =>$CaptionFontName, 'FontSize' =>$CaptionFontSize, 'R' => $Settings['R']+$SubCaptionColorFactor, 'G' => $Settings['G']+$SubCaptionColorFactor,
           ...
           if ( $Value >= $Settings['Start'] && $Value <= $Settings['End'] )
            {
             ...
             if ( $ValueDisplay == INDICATOR_VALUE_BUBBLE )
              {
               ...
               $this->pChartObject->drawFilledCircle($X1,$Y,$Radius+4,array( 'R' => $Settings['R']+20, 'G' => $Settings['G']+20,
                                                                             'B' => $Settings['B']+20));
             ...
             elseif( $ValueDisplay == INDICATOR_VALUE_LABEL )
              {
               $Caption = [];
               $Caption[] = array( 'Format'  =>array( 'R'     => $Settings['R'], 'G' => $Settings['G'], 'B' => $Settings['B'],
                                                      'Alpha' =>100),
                                   'Caption' =>$Value.$Unit);
