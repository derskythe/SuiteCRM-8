   ...
   public function draw2DPie($X,$Y,$Format= '')
    {
     ...
     foreach($Values as $Key => $Value)
      {
       if ( $Shadow )
        $Settings = array( 'R'     =>$this->pChartObject->ShadowR, 'G' =>$this->pChartObject->ShadowG, 'B' =>$this->pChartObject->ShadowB,
       ...
       else
        {
         ...
         $Settings = array( 'R'     => $Palette[$ID]['R'], 'G' => $Palette[$ID]['G'], 'B' => $Palette[$ID]['B'],
       ...
       if ( $DrawLabels && !$Shadow && !$SecondPass )
        {
         if ( $LabelColor == PIE_LABEL_COLOR_AUTO )
          { $Settings = array( 'FillR' => $Palette[$ID]['R'], 'FillG' => $Palette[$ID]['G'], 'FillB' => $Palette[$ID]['B'],
                               'Alpha' => $Palette[$ID]['Alpha']);}
         else
     ...
     foreach($Slices as $SliceID => $Plots)
      {
       ...
       if ( $Visible[$SliceID]['Start'] && isset($Plots[2])) /* Empty error handling */
        {
         $this->pChartObject->drawLine($Plots[2],$Plots[3],$Plots[2],$Plots[3]- $SliceHeight,array( 'R' => $Settings['R'], 'G' => $Settings['G'],
                                                                                                    'B' => $Settings['B']));
         $Border = [];
     ...
     foreach($Slices as $SliceID => $Plots)
      {
       ...
       if ( $Visible[$SliceID]['End'] )
        {
         $this->pChartObject->drawLine($Plots[(is_countable($Plots) ? count($Plots) : 0)-2],$Plots[(is_countable($Plots) ? count($Plots) : 0)-1],$Plots[(is_countable($Plots) ? count($Plots) : 0)-2],$Plots[(is_countable($Plots) ? count($Plots) : 0)-1]- $SliceHeight,array( 'R' => $Settings['R'], 'G' => $Settings['G'],
                                                                                                                                                                                                                                                                                'B' => $Settings['B']));
       ...
       foreach($Values as $Key => $Value)
        {
         ...
         if ( $Shadow )
          ...
          {
           if ( $Border )
            { $Settings = array( 'R'     => $Palette[$ID]['R']+30, 'G' => $Palette[$ID]['G']+30, 'B' => $Palette[$ID]['B']+30,
                                 'Alpha' => $Palette[$ID]['Alpha']); }
           else
            $Settings = array( 'R'     => $Palette[$ID]['R'], 'G' => $Palette[$ID]['G'], 'B' => $Palette[$ID]['B'],
                               'Alpha' => $Palette[$ID]['Alpha']);
     ...
     foreach($Data['Series'][$Data['Abscissa']]['Data'] as $Key => $Value)
      {
       ...
       $this->pChartObject->drawFilledRectangle($X+1,$Y+1,$X+$BoxSize+1,$Y+$BoxSize+1,array( 'R'     =>0, 'G' =>0, 'B' =>0,
                                                                                             'Alpha' =>20));
       $this->pChartObject->drawFilledRectangle($X,$Y,$X+$BoxSize,$Y+$BoxSize,array( 'R'           =>$R, 'G' =>$G, 'B' =>$B,
                                                                                     'Surrounding' =>20));
       if ( $Mode == LEGEND_VERTICAL )
        {
         $this->pChartObject->drawText($X+$BoxSize+4,$Y+$BoxSize/2,$Value,array( 'R'        =>$FontR, 'G' =>$FontG, 'B' =>$FontB, 'Align' =>TEXT_ALIGN_MIDDLELEFT, 'FontName' =>$FontName,
                                                                                 'FontSize' =>$FontSize));
         $Y=$Y+$YStep;
        }
       elseif ( $Mode == LEGEND_HORIZONTAL )
        {
         $BoxArray = $this->pChartObject->drawText($X+$BoxSize+4,$Y+$BoxSize/2,$Value,array( 'R'        =>$FontR, 'G' =>$FontG, 'B' =>$FontB, 'Align' =>TEXT_ALIGN_MIDDLELEFT, 'FontName' =>$FontName,
                                                                                             'FontSize' =>$FontSize));
   ...
   public function writePieLabel($X,$Y,$Label,$Angle,$Settings,$Stacked,$Xc=0,$Yc=0,$Radius=0,$Reversed=FALSE)
    {
     ...
     if ( !$Stacked )
     ...
     else
      {
       ...
       $LabelSettings = array( 'YTop' =>$YTop, 'YBottom' =>$YBottom, 'Label' =>$Label, 'Angle' =>$Angle, 'X1' =>$X, 'Y1' =>$Y, 'X2' =>$X2,
                               'Y2'   =>$Y2);
