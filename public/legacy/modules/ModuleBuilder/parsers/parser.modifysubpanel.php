...
class ParserModifySubPanel extends ParserModifyListView
{
    ...
    {
        parent::init($bean, $view_object_map, $submittedLayout);
        $GLOBALS ['log']->debug("in ParserModifySubPanel: module_name={$bean} child_module={$view_object_map}");
        $this->moduleName = $bean;
        $this->subPanelName = $view_object_map;
        ...
        if (array_key_exists(strtolower($this->subPanelName), $spd->layout_defs ['subpanel_setup'])) {
            ...
            $this->panel = $spd->load_subpanel($view_object_map, false);
    ...
    public function getAvailableFields()
    {
        ...
        if ($this->subPanelParentModule != null) {
            ...
            $GLOBALS ['log']->debug(
                'parser.modifylistview.php->getAvailableFields(): field_defs=' . print_r($this->availableFields, true)
            );
            foreach ($this->subPanelParentModule->field_defs as $key => $fieldDefinition) {
                ...
                if (!isset($lowerFieldList [$fieldName])) { // bug 16728 - check this first, so that other conditions (e.g., studio == visible) can't override and add duplicate entries
                    if ((empty($fieldDefinition ['source']) || $fieldDefinition ['source'] === 'db' || $fieldDefinition ['source'] === 'custom_fields') && $fieldDefinition ['type'] !== 'id' && strcmp(
                            $fieldName,
                            'deleted'
                        ) != 0 || (isset($def ['name']) && strpos(
                                (string) $def ['name'],
                                '_name'
                            ) != false) || !empty($def ['custom_type']) && (empty($fieldDefinition ['dbType']) || $fieldDefinition ['dbType'] !== 'id') && (empty($fieldDefinition ['dbtype']) || $fieldDefinition ['dbtype'] !== 'id') || (!empty($fieldDefinition ['studio']) && $fieldDefinition ['studio'] === 'visible')) {
                        $label =
                            (isset($fieldDefinition ['vname']))
                                ? $fieldDefinition ['vname']
                                : (isset($fieldDefinition ['label']) ? $fieldDefinition ['label']
