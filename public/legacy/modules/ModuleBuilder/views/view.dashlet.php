...
class ViewDashlet extends ViewListView
{
    public function __construct()
    {
        ...
        $this->fromModuleBuilder =
            isset($_REQUEST ['MB']) || (isset($_REQUEST['view_package']) && $_REQUEST['view_package'] && $_REQUEST['view_package'] !== 'studio');
    ...
    public function constructAjax()
    {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $ajax->addCrumb(
                translate('LBL_DASHLET', 'ModuleBuilder'),
    ...
    public function constructSmarty($parser)
    {
        ...
        $helpName =
            (isset($_REQUEST['view']) && $_REQUEST['view'] === 'dashletsearch') ? 'searchViewEditor' : 'listViewEditor';
        ...
        foreach ($parser->columns as $column => $function) {
            ...
            $groups [$GLOBALS ['mod_strings'] [$column]] =
                $parser->$function(
            ...
            foreach ($group as $fieldKey => $field) {
                if (isset($field ['width'])) {
                    if (str_ends_with((string) $field ['width'], '%')) {
                        $groups [$groupKey] [$fieldKey] ['width'] =
                            substr((string) $field ['width'], 0, strlen((string) $field ['width']) - 1);
                    }
                }
            }
        }
...
//        $imageHelp = SugarThemeRegistry::current()->getImage('help','',null,null,'.gif',$mod_strings['LBL_SECTION_HELP']) ;
        ...
        if (isset($this->searchlayout)) {
            $histaction =
                "ModuleBuilder.history.browse(\"{$this->editModule}\", \"{$this->editLayout}\", \"{$this->searchlayout}\")";
        }
        ...
        if (!$this->fromModuleBuilder) {
            ...
            $buttons [] =
                array( 'name'         => 'mbsavebtn',
                       'image'        => $imageSave,
                       'text'         => $GLOBALS ['mod_strings'] ['LBL_BTN_SAVE'],
                       'actionScript' => "onclick='studiotabs.generateGroupForm(\"edittabs\");ModuleBuilder.state.isDirty=false;ModuleBuilder.submitForm(\"edittabs\" )'" );
        }
        $buttons [] =
            array( 'name'         => 'historyBtn',
