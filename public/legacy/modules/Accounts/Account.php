...
class Account extends Company implements EmailInterface
{
    ...
    public function __construct()
    {
        ...
        $this->additional_column_fields =
            array( 'assigned_user_name',
                   ...
                   'parent_name',
                   'member_id' );
        $this->relationship_fields = array( 'opportunity_id' => 'opportunities',
                                            'bug_id'         => 'bugs',
                                            'case_id'        => 'cases',
                                            'contact_id'     => 'contacts',
                                            'task_id'        => 'tasks',
    ...
    public function clear_account_case_relationship($account_id = '', $case_id = '')
    {
        ...
        $query =
            "UPDATE cases SET account_name = '', account_id = '' WHERE account_id = " . $accountIdQuoted . ' AND deleted = 0 ' . $where;
    ...
    public function fill_in_additional_detail_fields() : void
    {
        ...
        if (empty($this->parent_name) && !empty($this->id)) {
            ...
            $query =
                'SELECT a1.name FROM accounts a1, accounts a2 WHERE a1.id = a2.parent_id AND a2.id = ' . $idQuoted . ' and a1.deleted=0';
    ...
    public function create_export_query(string $order_by, string $where) : array|string
    {
        ...
        foreach (explode('AND', $where) as $whereClause) {
            ...
            foreach ($this->field_defs as $field_def) {
                ...
                if (str_contains($newWhereClause, $needle)) {
